...
class NotepadTest(TestCase):
    ...
    def _open_notepad(self):
        ...
        for attempt in range(10):
            for w in Desktop(backend='uia').windows():
                try:
                    title = w.window_text()
                    if ("메모장" in title or "Notepad" in title) and w.handle not in existing_handles:
                        new_handle = w.handle
                        break
                except Exception:
                    pass
    ...
    def test_04_win_inspector_integration(self):
        ...
        notepad_found = any(
            "메모장" in w.get("title", "") or "Notepad" in w.get("title", "")
            for w in windows
        )
        ...
        self.assert_true("error" not in result, f"inspect_window 성공해야 합니다: {result.get('error', '')}")
    ...
    def test_05_close_without_save(self):
        ...
        self.step("윈도우 종료 확인")
        self.wait(0.5)
        closed = not self._notepad_process.poll() is None
        if not closed:
