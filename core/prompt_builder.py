...
class PromptBuilder:
    ...
    def _append_selenium_guide(self, parts: list[str]):
        ...
        parts.append("          //*[normalize-space(@title)=\"메뉴명\" or normalize-space(.)=\"메뉴명\"]")
        ...
        parts.append("            \"\"\"로케이터 우선순위 순으로 요소를 찾아 클릭.")
        parts.append("            visible_only=True : 뷰포트 내 실제 보이는 요소만 — 드롭다운/서브메뉴에 필수")
        parts.append("            visible_only=False: DOM 존재 시 즉시 반환 — 최초 페이지 로드 요소 등\"\"\"")
        parts.append("            import time as _t")
        ...
        parts.append("                        by, val = By.XPATH, f'//*[normalize-space(@title)=\"{value}\"]'")
        ...
        parts.append("                    if visible_only:")
        parts.append("                        # EC.visibility_of_element_located는 CSS hidden만 확인 →")
        parts.append("                        # x<0 위치의 사이드바 요소도 통과시키는 버그 존재.")
        parts.append("                        # find_elements + getBoundingClientRect: 뷰포트 안에 실제 보이는 요소만 선택")
        parts.append("                        _deadline = _t.time() + timeout")
        parts.append("                        el = None")
        parts.append("                        while _t.time() < _deadline:")
        parts.append("                            for _c in driver.find_elements(by, val):")
        parts.append("                                try:")
        parts.append("                                    _r = driver.execute_script(")
        parts.append("                                        'var r=arguments[0].getBoundingClientRect();'")
        parts.append("                                        'return {x:r.x,y:r.y,w:r.width,h:r.height};', _c)")
        parts.append("                                    if _r['w']>0 and _r['h']>0 and _r['x']>=0 and _r['y']>=0:")
        parts.append("                                        el = _c; break")
        parts.append("                                except Exception:")
        parts.append("                                    continue")
        parts.append("                            if el: break")
        parts.append("                            _t.sleep(0.1)")
        parts.append("                        if el is None:")
        parts.append("                            raise Exception(f'visible 요소 없음: {strategy}={value}')")
        parts.append("                    else:")
        ...
        parts.append("                    else:")
        parts.append("                        # ActionChains: mouseover/mouseenter/mousedown/click 이벤트 순서대로 발화")
        parts.append("                        # SPA의 '외부 클릭 = 메뉴 닫기' 리스너와 충돌 없이 안정적 클릭")
        parts.append("                        try:")
        ...
        parts.append("        ```")
        parts.append("        사용 예 — 일반/상위 메뉴: find_and_click(driver, [('xpath', '//*[normalize-space(@title)=\"공통업무\" or normalize-space(.)=\"공통업무\"]')])")
        parts.append("        사용 예 — 서브메뉴:       find_and_click(driver, [('xpath', '//*[normalize-space(@title)=\"실패사례\" or normalize-space(.)=\"실패사례\"]')], visible_only=True)")
        ...
        parts.append("        find_and_click(driver, [('xpath', '//*[normalize-space(.)=\"메뉴명\"]')])")
        parts.append("        # ✅ 올바른 예: visible_only=True → getBoundingClientRect로 뷰포트 안 요소만 선택")
        parts.append("        find_and_click(driver, [('xpath', '//*[normalize-space(.)=\"상위메뉴\"]')])")
        parts.append("        find_and_click(driver, [('xpath', '//*[normalize-space(.)=\"하위항목\"]')], visible_only=True)")
        parts.append("        ```")
