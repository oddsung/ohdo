...
class PromptBuilder:
    ...
    def _append_selenium_guide(self, parts: list[str]):
        ...
        parts.append("     ② 드롭다운/서브메뉴 클릭 시 반드시 find_and_click(..., visible_only=True)를 사용하세요.")
        parts.append("        presence_of_element_located: DOM에 존재하면 즉시 반환 — 사이드바 등 화면 밖 요소도 반환!")
        parts.append("        visibility_of_element_located: CSS hidden만 확인 — x<0 위치 요소도 '보임'으로 통과!")
        parts.append("        ✅ 올바른 방법: find_and_click(..., visible_only=True) — getBoundingClientRect()로 뷰포트 내 요소만 선택")
        ...
        parts.append("     ⑤ 로케이터 안정성 우선순위: id속성 > 결합 XPath(title+text) > aria-label > 텍스트내용")
        parts.append("        - ⚠️ 일부 SPA 요소는 HTML @title 속성이 없으므로 ('title','...') 단독 전략은 10초 타임아웃 후 실패합니다.")
        parts.append("          title과 text를 OR로 묶은 XPath를 사용해야 어느 경우에도 즉시 매칭됩니다:")
        parts.append("          //*[contains(normalize-space(@title),\"메뉴명\") or ./text()[contains(normalize-space(),\"메뉴명\")]]")
        ...
        parts.append("        - 아이콘 폰트 PUA 문자(\\uE000~\\uF8FF) 포함 텍스트 → contains()로 자동 매칭")
        parts.append("        - 새 탭/창으로 열리는 콘텐츠 → window_handles 자동 전환")
        parts.append("        - iframe 내부 요소 → switch_to.frame() 자동 탐색")
        parts.append("        ```python")
        parts.append("        def find_and_click(driver, locators, timeout=10, visible_only=False):")
        parts.append("            \"\"\"범용 웹 요소 클릭 함수.")
        parts.append("            새 탭/창·iframe 자동 탐색, PUA 아이콘 문자 자동 처리, off-canvas JS click 지원.\"\"\"")
        ...
        parts.append("                bm = {'id': By.ID, 'css': By.CSS_SELECTOR, 'xpath': By.XPATH}")
        parts.append("                if strat == 'title':")
        parts.append("                    v = _re.sub(r'[\\uE000-\\uF8FF]', '', val).strip()")
        parts.append("                    return By.XPATH, f'//*[contains(normalize-space(@title),\"{v}\")]'")
        parts.append("                if strat == 'text':")
        parts.append("                    v = _re.sub(r'[\\uE000-\\uF8FF]', '', val).strip()")
        parts.append("                    return By.XPATH, f'//*[not(self::script)][not(self::style)][./text()[contains(normalize-space(),\"{v}\")]]'")
        parts.append("                if strat == 'xpath':  # xpath 전략: contains() 내부 값 앞뒤 공백 자동 제거")
        parts.append("                    val = _re.sub(r'contains\\(([^,]+),\"([^\"]+)\"\\)',")
        parts.append("                                  lambda m: f'contains({m.group(1)},\"{m.group(2).strip()}\")', val)")
        ...
        parts.append("                            try:")
        parts.append("                                _r = ctx.execute_script('var r=arguments[0].getBoundingClientRect();return {x:r.x,y:r.y,w:r.width,h:r.height};', _c)")
        parts.append("                                if _r['w']>0 and _r['h']>0 and _r['x']>-10 and _r['y']>-10: el=_c; break")
        ...
        parts.append("                else:")
        parts.append("                    el = WebDriverWait(ctx, t).until(EC.presence_of_element_located((by, val)))")
        parts.append("                r = ctx.execute_script('var r=arguments[0].getBoundingClientRect();return {x:r.x,y:r.y};', el)")
        parts.append("                if r['x']<0 or r['y']<0: ctx.execute_script('arguments[0].click()',el)")
        parts.append("                else:")
        parts.append("                    try: ActionChains(ctx).move_to_element(el).click().perform()")
        parts.append("                    except Exception:")
        parts.append("                        try: el.click()")
        parts.append("                        except Exception: ctx.execute_script('arguments[0].click()',el)")
        ...
        parts.append("                try: return _click(driver, by, bval, visible_only, timeout)  # 현재 컨텍스트")
        ...
        parts.append("                            return _click(driver, by, bval, visible_only, min(timeout,4))")
        parts.append("                        except Exception:")
        ...
        parts.append("        ```")
        parts.append("        사용 예 — 일반/상위 메뉴: find_and_click(driver, [('xpath', '//*[contains(normalize-space(@title),\"공통업무\") or ./text()[contains(normalize-space(),\"공통업무\")]]')])")
        parts.append("        사용 예 — 서브메뉴:       find_and_click(driver, [('xpath', '//*[./text()[contains(normalize-space(),\"실패사례\")]]')], visible_only=True)")
        parts.append("        사용 예 — id로 찾기:     find_and_click(driver, [('id', 'year')])  # iframe/새탭 자동 탐색")
        ...
        parts.append("           이유: @title에 PUA 아이콘 문자(\\uE000~\\uF8FF)가 앞에 붙을 수 있음")
        parts.append("        ② 텍스트 매칭: ./text()[contains(normalize-space(),\"버튼명\")]")
        parts.append("           이유: normalize-space(.)는 자식 요소의 텍스트까지 포함하여 조상 요소도 매칭됨.")
        parts.append("                ./text()는 직접 텍스트 노드만 확인하므로 정확한 요소를 찾음.")
        parts.append("        ```")
        parts.append("        # ❌ 절대 사용 금지")
        parts.append("        normalize-space(@title)=\"조회\"                     # PUA 문자로 인해 실패")
        parts.append("        contains(normalize-space(.),\"버튼명\")              # 조상 div/nav도 매칭 → 잘못된 요소 클릭")
        parts.append("        # ✅ 올바른 패턴")
        parts.append("        contains(normalize-space(@title),\"조회\")           # @title PUA 대응")
        parts.append("        ./text()[contains(normalize-space(),\"사례등록\")]   # 직접 텍스트 노드만 매칭")
        parts.append("        ```")
        parts.append("        XPath 작성 시 전체 패턴:")
        parts.append("        '//*[contains(normalize-space(@title),\"버튼명\") or ./text()[contains(normalize-space(),\"버튼명\")]]'")
        parts.append("     ⑦ 🌟중요(드롭다운·서브메뉴·팝업 클릭): 클릭 후 나타나는 모든 하위 항목은 반드시")
        parts.append("        visible_only=True를 사용하세요. 많은 웹 앱이 사이드바/다른 메뉴의 항목을")
        parts.append("        DOM에 미리 생성해 두기 때문에, XPath가 뷰포트 밖(x<0) 숨겨진 요소를 먼저 찾고")
        parts.append("        JS click으로 잘못 클릭하는 버그가 발생합니다 (아무 일도 안 일어남):")
        parts.append("        ```python")
        parts.append("        # ❌ 잘못된 예: XPath가 사이드바의 숨겨진 요소(x=-220)를 먼저 반환 → JS click → 아무 변화 없음")
        parts.append("        find_and_click(driver, [('xpath', '//*[./text()[contains(normalize-space(),\"메뉴명\")]]')])")
        parts.append("        # ✅ 올바른 예: visible_only=True → getBoundingClientRect로 뷰포트 안 요소만 선택")
        parts.append("        find_and_click(driver, [('xpath', '//*[./text()[contains(normalize-space(),\"상위메뉴\")]]')])")
        parts.append("        find_and_click(driver, [('xpath', '//*[./text()[contains(normalize-space(),\"하위항목\")]]')], visible_only=True)")
        parts.append("        ```")
        parts.append("        적용 범위: 드롭다운 메뉴, 팝업 내 버튼, 탭 전환 후 항목, 아코디언 하위 항목 등")
        parts.append("        최초 페이지 로드 요소(헤더, 로그인 버튼 등)는 visible_only=False(기본값) 유지")
        parts.append("     ⑩ 🌟중요(DOM 컨텍스트 활용 — 최적 로케이터 결정): 요소 선택 시 제공되는")
        parts.append("        outerHTML/CSS 클래스/XPath/부모 HTML을 분석해 최적 전략을 선택하세요:")
        ...
        parts.append("        4. DOM에 동일 텍스트의 숨겨진 요소가 여럿 존재할 때(SPA 드롭다운/아코디언 등):")
        ...
        parts.append("        def click_visible_element(driver, css_selector, text, parent_selector=None, timeout=10):")
        parts.append("            \"\"\"CSS 선택자로 후보를 좁히고, 화면에 보이는 요소만 클릭.")
        parts.append("            parent_selector: 클릭 핸들러가 있는 조상 CSS 선택자 (예: '.menu-item', 'li')")
        parts.append("            DOM 컨텍스트의 CSS 클래스와 부모 HTML을 참고해 인자를 채우세요.\"\"\"")
        parts.append("            from selenium.webdriver.common.action_chains import ActionChains")
        ...
        parts.append("                                    ? items[i].closest(arguments[2]) : items[i];")
        ...
        parts.append("                if el:")
        parts.append("                    ActionChains(driver).move_to_element(el).click().perform()")
        parts.append("                    return el")
        parts.append("                time.sleep(0.2)")
        parts.append("            raise Exception(f'{text} 요소를 찾지 못했습니다 ({css_selector})')")
        parts.append("        # 사용 예: 제공된 DOM 컨텍스트(CSS 클래스·부모 HTML)를 참고해 인자 결정")
        parts.append("        # css_selector  → 선택된 요소의 CSS 클래스 (예: '.menu-label', '.item-text')")
        parts.append("        # parent_selector → 클릭 핸들러가 있는 부모 선택자 (예: '.menu-item', 'li')")
        parts.append("        click_visible_element(driver, '.menu-label', '메뉴명', parent_selector='.menu-item')")
        parts.append("        ```")
        parts.append("     ⑧ 🌟중요(TAB 키 후 입력): TAB으로 포커스를 이동한 다음에는 원래 요소 변수가 아닌")
        parts.append("        반드시 driver.switch_to.active_element로 현재 포커스 요소에 입력하세요.")
        parts.append("        원래 요소 변수는 포커스 이동과 무관하게 그 요소에만 전송되므로 다음 필드에 입력이 안 됩니다:")
        parts.append("        ```python")
        ...
        parts.append("        login_input.send_keys('아이디')")
        parts.append("        login_input.send_keys(Keys.TAB)")
        parts.append("        driver.switch_to.active_element.send_keys('비밀번호')  # PW 필드에 정확히 입력")
        parts.append("        driver.switch_to.active_element.send_keys(Keys.ENTER)")
        parts.append("        ```")
        parts.append("     ⑨ 🌟중요(로그인·폼 제출 후 대기): Enter/클릭으로 로그인하거나 페이지가 이동하면")
        parts.append("        반드시 다음 요소가 나타날 때까지 기다린 후 클릭하세요. 즉시 클릭하면 요소를 찾지 못합니다:")
        parts.append("        ```python")
        parts.append("        # 로그인 Enter 전송 후 — 다음 페이지의 특정 요소 대기")
        parts.append("        element.send_keys(Keys.ENTER)")
        parts.append("        WebDriverWait(driver, 15).until(EC.presence_of_element_located((By.XPATH, '//*[@title=\"메뉴명\"]')))")
