...
from PySide6.QtGui import QAction, QIcon, QFont, QKeySequence, QShortcut
...
class AsyncSignals(QObject):
    """스레드 간 통신 시그널"""
    ai_response_ready = Signal(dict)    # AI 응답 수신
    step_executed = Signal(dict)         # 스텝 실행 완료
    log_message = Signal(str)            # 로그 메시지
    error_occurred = Signal(str)         # 에러 발생
    # 블럭 실행 전용
    block_step_started = Signal(int)     # 블럭 스텝 시작 (step_id)
    block_step_done = Signal(dict)       # 블럭 스텝 완료
    blocks_finished = Signal()           # 블럭 실행 모두 완료 (UI 복원 트리거)
    kernel_status_changed = Signal()     # 커널 상태 변경
