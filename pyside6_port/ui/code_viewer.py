...
class StepCard(QFrame):
    ...
    insert_requested = Signal(int)      # after_step_id
    move_up_requested = Signal(int)     # step_id
    move_down_requested = Signal(int)   # step_id
...
class BlockViewWidget(QWidget):
    ...
    # 커널 재시작 요청
    kernel_reset_requested = Signal()
    # N번 스텝부터 실행 요청 (0 = 라이브러리 블럭만 재실행)
    run_from_step_requested = Signal(int)
    # N번 스텝 단독 실행 (Phase 1 — 다음 step 안 함)
    run_single_step_requested = Signal(int)
    # Step 후 대기시간 변경 (step_id, new_wait_ms or None)
    wait_changed = Signal(int, object)
    # 중지 요청
...
class CodeViewer(QWidget):
    ...
    run_code_requested = Signal(str)           # 코드 실행 요청 (기존 모드)
    stop_code_requested = Signal()             # 코드 중지 요청
    step_delete_requested = Signal(int)        # 스텝 삭제 요청
    step_insert_requested = Signal(int)        # 스텝 삽입 요청 (after_step_id)
    step_move_requested = Signal(int, str)     # 스텝 이동 (step_id, "up"/"down")
    step_code_edited = Signal(int, str)        # 사용자가 코드 직접 수정 (step_id, new_code)
    # 블럭 뷰 전용 시그널
    run_from_step_requested = Signal(int)      # N번 스텝부터 블럭 실행 요청
    run_single_step_requested = Signal(int)    # N번 스텝 단독 실행 (Phase 1)
    wait_changed = Signal(int, object)         # wait 변경 (step_id, ms or None)
