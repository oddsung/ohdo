...
class AIIntegrationTest(TestCase):
    ...
    def _validate_generated_code(self, code: str, context: str = ""):
        """생성된 코드의 기본 품질 검증"""
        self.assert_true(len(code) > 10,
                         f"코드가 충분히 생성되어야 합니다 (현재 {len(code)}자) {context}")
        self.assert_contains(code, "import",
                             f"import문이 포함되어야 합니다 {context}")
        ...
        return {"has_error_handling": has_error_handling, "has_print": has_print}
    ...
    def test_01_simple_code_generation(self):
        ...
        prompt = builder.build_step_prompt(
            session=session,
            user_request="현재 날짜와 시간을 출력하는 코드를 만들어줘"
        )
        ...
        self.assert_true(len(response.code) > 0,
                         f"코드가 추출되어야 합니다. 원본 응답: {response.text[:200]}")
        quality = self._validate_generated_code(response.code, "(날짜 출력)")

        self.step("코드 실행")
        sandbox = self._make_sandbox()
        result = sandbox.execute(response.code)
        self.assert_true(result.success,
                         f"코드 실행 성공해야 합니다. 에러: {result.error}")
    ...
    def test_02_subprocess_code_generation(self):
        ...
        self.assert_contains(response.code, "subprocess",
                             "subprocess가 사용되어야 합니다")
    ...
    def test_03_accumulated_code_preservation(self):
        ...
        prompt = builder.build_step_prompt(
            session=session,
            user_request="현재 디렉토리의 파일 목록도 출력해줘"
        )
        ...
        self.assert_contains(response.code, "os.getcwd()",
                             "이전 스텝의 os.getcwd()가 유지되어야 합니다")
        self.assert_contains(response.code, "현재 디렉토리",
                             "이전 스텝의 print 내용이 유지되어야 합니다")

        self.step("새 기능 추가 확인")
        has_listdir = "listdir" in response.code or "scandir" in response.code or "glob" in response.code
        self.assert_true(has_listdir,
                         "파일 목록 기능이 추가되어야 합니다 (listdir/scandir/glob)")
    ...
    def test_04_error_recovery_code_generation(self):
        ...
        executable_lines = [
            line for line in response.code.split('\n')
            if line.strip() and not line.strip().startswith('#')
        ]
        executable_code = '\n'.join(executable_lines)
        has_broken_path = "/this_directory_does_not_exist_12345" in executable_code
        if has_broken_path:
            self.log(f"[WARN] 실행 코드에 잘못된 경로가 남아있지만, 실행 결과로 판단합니다")

        self.step("수정된 코드 실행")
        sandbox = self._make_sandbox()
        result = sandbox.execute(response.code)
        self.assert_true(result.success,
                         f"수정된 코드는 실행 성공해야 합니다. 에러: {result.error}")
        self.log(f"출력: {result.output[:300]}")
...
- [StatusBar] "Ln 1, Col 1" @ (0,550)"""
        ...
        self.assert_true(has_pywinauto or has_pyautogui,
                         "자동화 라이브러리 (pywinauto 또는 pyautogui)가 사용되어야 합니다")

        # 메모장 연결 시도
        has_notepad_connect = ("메모장" in code or "Notepad" in code or "notepad" in code)
        self.assert_true(has_notepad_connect,
                         "메모장에 연결하는 코드가 있어야 합니다")

        # 텍스트 입력
        has_text_input = ("안녕하세요" in code or "set_text" in code or
                          "type_keys" in code or "send_keys" in code)
        self.assert_true(has_text_input,
                         "텍스트 입력 코드가 있어야 합니다")
    ...
    def test_06_element_context_code_generation(self):
        ...
        self.assert_true(has_element_ref,
                         "요소의 automation_id 또는 이름이 코드에 반영되어야 합니다")
    ...
    def test_07_browser_element_generates_selenium(self):
        ...
        self.assert_true(has_selenium,
                         "브라우저 요소에 대해 Selenium 코드가 생성되어야 합니다")
    ...
    def test_08_full_pipeline_safe_code(self):
        ...
        self.assert_contains(result.output, "55",
                             "1부터 10까지의 합 55가 출력되어야 합니다")
    ...
    def test_09_multi_step_pipeline(self):
        ...
        session.steps = [
            {"step_id": 1, "status": "completed", "generated_code": response1.code}
        ]
        ...
        # 기존 코드 유지 확인
        self.assert_contains(response2.code, "사과",
                             "스텝1의 리스트 내용이 유지되어야 합니다")

        result2 = sandbox.execute(response2.code)
        self.assert_true(result2.success, f"스텝2 실행 성공: {result2.error}")
        self.assert_contains(result2.output, "포도",
                             "스텝2에서 '포도'가 추가되어 출력되어야 합니다")
