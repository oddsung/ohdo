...
> 마지막 업데이트: 2026-08-14 (마흔다섯 번째 작업 — 자세한 변경은 §5 변경 이력 + §11~§92 인계 노트 참조). baseline: **core 244/244 + scenarios 73/73 + recording_fixtures 2/2 + ruff 그린 + GitHub Actions CI 3잡 전체 그린** (§92 신규 — **packaged 코드 실행 성립: frozen 파이썬 런너 모드(v1.0 블로커 fix, 사용자 승인 core 수정 3파일) → v0.1.3**: 발단 = 사용자 환경점검 화면(패키지 전부 missing). 조사 — packaged 앱의 sys.executable = 브리지 exe(파이썬 아님)인데 ① ExecutionKernel 이 [exe, -u, kernel_worker.py] 스폰(argparse 즉사 + kernel_worker.py 는 번들에 파일로 부재 — 실증) ② CodeSandbox 동일 + pip 자동설치 불가 ③ 환경스캐너 [exe, -c, import x] 전패 → **설치본에서 스텝 실행 원천 불가 + 전 패키지 missing 오탐**(dev 는 .venv 파이썬이라 잠복). fix: **api_server.__main__ 런너 모드** — --run-kernel-worker(runpy 로 번들 core.kernel_worker 실행)/--run-script <path>, 비-frozen 서도 동작(test_249 가 기능 검증), stdout line_buffering(-u 동등). core: execution_kernel/workflow_engine 은 frozen∧자기자신일 때만 런너 경로(비-frozen 스폰 보존 — v2 무영향), CodeSandbox frozen pip 스킵(pkg_resources 부재 오탐 차단), environment_scanner 는 frozen 자기스캔을 in-process find_spec 으로 + PySide6 를 frozen 필수에서 제외(번들 의도적 미포함) + 파이썬 버전 "(bundled)" 표기. spec: core.kernel_worker hiddenimport(경로 참조라 정적분석 미탐 — 이것도 §92 전까지 번들 누락이었음). **frozen 실증**: 재freeze 후 exe --run-script 로 pyautogui/pywinauto/mss/PIL import+실행 성공(FROZEN_RUNNER_OK), --run-kernel-worker 4s 생존. test_249 +1(core **244/244**)+scenarios 73/73+fixtures 2/2+ruff 그린. **v0.1.3 릴리스**(§91 새 아이콘 동시 탑재, release4 출력 — release/·3 는 IDE 잠금). 잔여 실측: 0.1.2→0.1.3 자동업데이트 후 **설치본에서 세션 실행**(진짜 최종 검증) + 환경점검 표시 정상화 + 새 아이콘 확인.) (§91 신규 — **앱 아이콘 교체(시안 A "oh 워드마크 배지" 채택, 사용자 선택)**: 기존 = Electron 기본 아이콘(원자 궤도, 미설정 상태). 시안 4종(A 워드마크/B 커서+스파크/C 말풍선 프롬프트/D o→o 경로) 아티팩트 제시 → A 채택. 구현: 시안 SVG 좌표(96 그리드)를 PIL 8x 슈퍼샘플(1024) 드로잉 — 대각 블러플 그라디언트(#6470f5→#4550cf) 라운드 배지 + 기하 o/h 글리프 → LANCZOS 다운스케일로 **ICO 7프레임(256~16)** + PNG(512) 생성(스크립트 scratchpad/gen_icon.py, 산출물 desktop_v3/build/icon.{ico,png} 커밋). package.json build.win.icon 지정. 검증: --dir 빌드에서 "default Electron icon is used" 경고 소멸 + **exe 에서 아이콘 추출해 배지 확인**(ExtractAssociatedIcon). .gitignore desktop_v3/release*/ 로 일반화. 다음 릴리스(0.1.3 — frozen 커널 fix 승인 대기와 동시 탑재 권장)부터 설치본/작업표시줄/시작메뉴에 반영. D안 모티브는 실행 FX/로딩 확장 후보로 보류.) (§90 신규 — **About 다이얼로그 + 수동 업데이트 확인(사용자: "버전 확인 메뉴가 없어 업데이트 설치 확인이 어렵다") → v0.1.2**: ① 타이틀바 햄버거 메뉴에 **"ohdo 정보"** — 앱 버전(`app:get-info` IPC: version/electron/platform/packaged) + Python 브리지 버전(/health) + 런타임 표시, GitHub 링크(`shell:open-repo` — openExternal 을 저장소 URL 한정으로만 노출) ② **"업데이트 확인" 버튼** — `updater:manual-check` IPC(핸들러는 항상 등록, dev 면 status="dev" 반환으로 renderer throw 방지; packaged 면 checkForUpdates → 최신/available(autoDownload 시작→§82 배너 연결)/error 상태 표시) ③ i18n `about.*` ko/en. tsc·build 그린, version 0.1.1→0.1.2, 커밋 3dd7e97(main 동기). **릴리스**: `release/` 가 또 Antigravity IDE 에 잠겨(§84b 재발) **`-c.directories.output=release2` 우회 빌드** 후 `gh release create v0.1.2`(latest.yml 0.1.2 확인). .gitignore 에 release2/ 추가(폴더 삭제는 IDE 잠금으로 보류). 설치본(0.1.1) 재시작으로 업데이트 체크 트리거 — **사용자 검증 시나리오: 배너 → 재시작 → 메뉴 "ohdo 정보" 에서 v0.1.2 확인 + "업데이트 확인" → "최신 버전" 표시**. 앱 재시작 시에만 자동 체크(부팅 10s + 4h 간격)라는 점도 About 의 수동 확인 도입 사유.) (§89 신규 — **세션 이름 UX 3건(사용자 실측 요청) + v0.1.1 릴리스로 자동업데이트 실검증 완료**: ① **세션 rename 무반응 실버그** — 사이드바 연필 버튼이 `window.prompt` 사용, **Electron 은 prompt() 미지원**이라 조용히 무동작(§75 의 confirm 계열과 동족) → **행 내 인라인 편집**으로 교체(SessionRow editing state, Enter 저장/Esc·blur 취소, rename 라우트/mutation 재사용) ② **새 세션 이름 입력 다이얼로그** — 자동 추천 이름 즉시 생성 대신 신규 `NewSessionDialog`(추천 이름 프리필+전체선택, Enter 생성) 경유. 사이드바 +/타이틀바 메뉴/팔레트/빈 화면/Ctrl+N 전 트리거 공용(`uiStore.newSessionOpen`), 산재하던 createSession mutation 3곳(App/팔레트/사이드바) 제거·일원화 ③ **타이틀바 SquarePen 새세션 퀵버튼 제거**(사이드바 + 와 중복, 사용자 결정 — 메뉴 항목은 유지). i18n `common.create`/`session.namePlaceholder` 추가·`renamePrompt` 제거. tsc 그린, **version 0.1.0→0.1.1**, 커밋 e51547e(main 동기). **§89b — 자동업데이트 실검증(§82 마지막 미검증 조각)**: `npm run dist`(재freeze 불필요 — 브리지 무변경) → `gh release create v0.1.1`(draft 아님, 자산 3종) → 설치된 0.1.0 실행 → **시작 ~40초 만에 183MB 자동 다운로드 완료**(`%LOCALAPPDATA%\ohdo-desktop-v3-updater\pending\` — ⚠️ 캐시 디렉터리명은 productName 아닌 package.json name 기반) → 배너 표시 → **정상 종료 시 autoInstallOnAppQuit 자동 설치 → 설치본 0.1.1 확인**. GitHub Releases 경유 실배포 체인 전체 입증. ROADMAP 임계경로 자동업데이트 항목 [x]. 잔여: 0.1.1 에서 UI 3건 사용자 실측(재실행 상태) + 기능 실측 계속.) (§88 신규 — **오픈코어 경계 복원 + README 제품 중심 재구성**(사용자: "정식 출시 대상은 desktop_v3 인데 다른 프로그램까지 전부 공유되나?" → 권장안 승인): public 전환으로 **SaaS 트랙까지 공개된 모순**(ROADMAP/CONTRIBUTING 의 "SaaS 코드 비공개" 선언과 불일치) 해소. ① **private repo `oddsung/ohdo-saas` 신규** — packages/(backend 71파일+web)+agent 인스톨러 파트 스냅샷 push(b178426 기준, README 에 출처/재개 가이드; 상세 이력은 공개 repo git history 참조로 갈음) ② **public 트리 제거 110파일**(디스크 보존, --cached): packages/ 전부, agent/ 중 SaaS 파트(**`agent/bridge.py`+`__init__.py` 는 데스크톱 Phase 1 스켈레톤·scenarios test_73 대상이라 공개 잔류**), legacy_pyqt6/(deprecated — 이력·로컬 보관), 루트 디버깅 잔재 5파일(uia_test/uia_desktop_test/uia_dump/test_click_diagnosis/sandbox_test — gitignore 기선언 잔존분) ③ .gitignore 경계 블록(packages/·agent/*+!bridge 예외·legacy_pyqt6/) ④ 문서 — CLAUDE.md legacy 문구, ROADMAP §3.0 표(SaaS 3행 위치→private ohdo-saas)+§10 ⑤ **README(en/ko) desktop_v3 중심 재구성**: 신규 "다운로드" 섹션(Releases latest 링크 + **SmartScreen 미서명 안내** — §81 결정 이행) → "소스에서 실행(개발자)" 이 desktop_v3(npm run dev) 우선 + PySide6 는 fallback 표기, 프로젝트 구조 트리 갱신(desktop_v3/api_server/devloop 반영·legacy 제거), 라이선스 절에 **장기 과금 방향(개인 무료/기업·상용 유료)** 명시, Gemini CLI 하드코딩 문구→"OpenAI 호환(DeepSeek)/CLI AI 앱 내 선택". 릴리스 산출물은 원래 desktop_v3 만 패키징이라 무영향. 이력에는 제거분 잔존(재작성 비권장 기존 판단 유지) — 향후 상용 SaaS 개발은 ohdo-saas 에서.) (§87 신규 — **CI 그린화(2026-05-18 이후 첫 전체 그린) — public 첫인상 복구**: 사용자가 main 푸시 후 CI 실패 메일 보고. 조사 — **CI 는 5/18부터 계속 빨간불**(로컬만 그린, 아무도 안 봄), 원인은 제품 버그 0건 + **환경 의존성 3층**. 3회 반복 수정(각 라운드가 다음 층을 노출): **1라운드** ① 러너 collect_environment 가 pyautogui import 실패(headless `KeyError: DISPLAY`)로 즉사 → 예외 흡수 ② test_71/75 가 gitignore 로컬 전용 config/settings.json 을 직접 read → 존재 시에만 검증/default 가드는 default_settings.json 으로 교체 ③ test_207/210 이 로컬 data/ 의 기존 세션 가정 → 세션 자급(생성→검증→정리). → Windows 잡 그린. **2라운드**(Ubuntu 가 3개월 만에 처음 스위트 실행, 17건): ④ ci.yml 에 xvfb-run(DISPLAY 10건) ⑤ `PYTHON_KEYRING_BACKEND=keyrings.alt` plaintext(CI 한정 — headless 에 SecretService 없음, vault 5건; 실백엔드는 Windows 잡 커버) ⑥ test_210 없는 세션 start 는 비-win32 에서 501 이 계약(win32 가드가 조회보다 먼저) ⑦ test_246 IME 상태 클리어는 win32 한정. **3라운드**: ⑧ mouseinfo 가 tkinter 부재 시 **sys.exit(SystemExit)** → `except Exception` 을 뚫고 러너 사망 → SystemExit 흡수 + ci.yml apt `python3-tk`(CI venv 는 시스템 python 기반 — 트레이스백 /usr/lib/python3.12 로 확인). 결과: **Lint+Windows+Ubuntu 3잡 그린**, README CI 배지 자동 그린 전환. 로컬 Windows core 243/243 무회귀. 커밋 85fd120→6172ca9→ea8c70d(main 동기). 교훈: CI 실패 메일 방치 금지 — 이제 public 이라 체크가 첫인상.) (§86 신규 — **장기 과금 방향 확정(사용자) + 출시 마무리 수순**: ① 과금 방향 = **개인 무료 / 기업·상용 유료** — ROADMAP §1 라이선스 전략에 기록(AGPL 은 상업 사용 금지 불가 → 실현 = 듀얼 라이선스 + 기업 타깃 Pro/SaaS 기능 유료화, 세부 라인 Phase 2 확정 유지). ② §85 fix 는 별도 커밋 b3b1c9b(외부 툴 자동 메시지 — 내용은 GPU fallback+productName+BUILD.md/handoff §85)로 기푸시 확인. ③ main ff-병합 + push(origin/main=1cb6270). **§86b — repo PUBLIC 전환 + 첫 릴리스 공개(2026-08-14, 사용자 "계속" 지시로 실행)**: `gh repo edit --visibility public` → github.com/oddsung/ohdo **PUBLIC**(Stars 0에서 측정 시작) → `gh release edit v0.1.0 --draft=false` — **일반 릴리스로 공개**(pre-release 표시는 electron-updater 기본 설정이 무시해 향후 0.1.1 업데이트가 안 잡히므로 의도적 미사용; 발표 전이라 노출 무해) → **비인증 `releases/latest/download/latest.yml` 접근 검증 = 자동업데이트 경로 활성**. ROADMAP §3.0 임계경로 갱신(빌드·공개 계열 [x], 잔여 = 자동업데이트 실검증(0.1.x 패치 시)·기능 실측(BUILD.md §4+§58~§66)·v1.0 범프+발표). **📍 다음: ① 사용자 설치본 기능 실측 ② 다음 fix 묶음을 0.1.1 로 릴리스해 자동업데이트 배너 실검증 ③ README SmartScreen 안내 문구+공개 첫인상 정리 ④ v1.0 범프+발표(Show HN/Reddit)+사용자 5명.**) (§85 신규 — **설치본 "더블클릭 무반응" 규명 + GPU 샌드박스 자동 fallback**: 첫 NSIS 설치 실측에서 앱이 아무 반응 없이 죽음(사용자 보고). **디버깅 함정 2개** — 이 세션(VS Code) 상속 env `ELECTRON_RUN_AS_NODE=1` 이 packaged exe 를 Node 로 돌려 즉시 exit 0(초기 테스트 전부 무효), `CHROME_CRASHPAD_PIPE_NAME` 도 오염 후보 — packaged 실행 테스트 전 반드시 두 env 제거. 진짜 증상: main 은 정상(브리지 spawn 로그 OK)이나 **GPU 프로세스가 STATUS_BREAKPOINT(-2147483645) 로 9연속 크래시 → Chromium FATAL "GPU process isn't usable" → 앱 종료**. 격리 실험: dev electron ✓ / win-unpacked ✓ / **설치본만 ✗** / 파일 해시 전수 동일 / 설치폴더를 TEMP 로 복사(ACL 재상속)하면 ✓ / `--disable-gpu-sandbox` ✓ → **원인 = `%LOCALAPPDATA%\Programs\ohdo` 경로 한정 보안 컨텍스트**(회사 도메인 머신 — 보안 에이전트의 경로 기반 개입 의심)가 GPU 샌드박스 프로세스만 죽임. fix(main/index.ts): **GPU 샌드박스 자동 fallback** — 기본은 풀 샌드박스, `child-process-gone`(type=GPU, crashed 계열) 3회에 userData 마커 기록 + `app.relaunch()`+`quit()`(before-quit 로 브리지 정리), 기동 시 마커 존재하면 `--disable-gpu-sandbox` 적용(문제 머신만 자가 복구, 마커 삭제로 원복). 부수: package.json 최상위 `productName: "ohdo"` 추가 — Electron `app.getName()` 이 이걸 읽어 **userData = %APPDATA%\ohdo**(기존엔 name 기반 ohdo-desktop-v3 로 문서와 불일치; 구 경로 잔재는 무시 가능). **검증**: 재빌드→무인 재설치(`/S`)→1차 기동 GPU 3크래시→자가 재기동→**20s 후 ohdo×4+bridge 생존 + 마커 생성** 확인. draft 릴리스 v0.1.0 자산 3종을 fixed 빌드로 `--clobber` 교체(16:33). tsc·electron-vite build 그린. **📍 남은 순서: ① 사용자 UI 실측(창 표시 중 — BUILD.md §4 체크리스트) ② main 병합→repo public 전환 ③ draft publish(`gh release edit v0.1.0 --draft=false`) ④ 정식 v1.0 버전 범프.**) (§84 신규 — **첫 `dist:publish` 시도 실측 + 근접사고 차단**: 사용자 실행 `npm run dist:publish` 가 winCodeSign 캐시 추출의 **darwin dylib symlink 생성 실패**(BUILD.md §6 기지 이슈, 비관리자+개발자 모드 OFF — 레지스트리 `AllowDevelopmentWithoutDevLicense` 미존재 확인)로 4회 재시도 끝 중단. **조치 중 근접사고 발견**: `desktop_v3/build/pybridge/` 가 **5/31 §64 잔재**(옛 spec) — `_internal/config/settings.json` 에 **실 API 키 동봉** + 신형 Electron 의 `--config-dir` 인자를 몰라 부팅 불능. 그대로 빌드 성공했다면 릴리스로 키 유출이었음. fix: ① §81 신규 spec 으로 재freeze → 번들 config = default_settings+prompts 만 확인 ② frozen 단독 스모크 — READY 마커·/health 200·**PUT /settings 가 --config-dir 쪽 settings.json 에 기록**(§81 frozen end-to-end 검증) ③ `build/pybridge/` 교체 + winCodeSign 캐시(오염 추출물) 삭제 ④ 겸사 발견한 frozen `--help` 크래시(cp949 콘솔 × docstring `→`) fix — `__main__.main()` 서두에서 stdout/stderr `reconfigure(encoding="utf-8", errors="replace")`(READY 마커는 ASCII 로 무영향, 한글/유니코드 로그 크래시 클래스 제거), core 243/243·ruff 그린 ⑤ BUILD.md §2 에 "옛 pybridge 잔재로 dist 금지" 경고 추가. **§84b 같은 날 해소 — 첫 릴리스 업로드 성공**: 2차 시도는 winCodeSign 통과 후 `release\win-unpacked\resources\app.asar` 잠금으로 실패 → Restart Manager(RmGetList)로 잠금 주인 식별 = **Antigravity IDE**(ohdo 폴더를 열어둔 다른 IDE 의 인덱서가 빌드 산출물 점유). 사용자가 IDE 닫고 재실행(`$env:GH_TOKEN=(gh auth token)`) → **업로드 성공: GitHub Releases `v0.1.0` draft** — `ohdo-0.1.0-setup.exe`(183MB)+`latest.yml`+`.blockmap` 3자산 확인(gh api). **freeze→동봉→NSIS→Releases 파이프라인 첫 end-to-end 완주**. 교훈: 빌드 전 다른 IDE/에디터가 release/ 를 물고 있지 않은지 확인(BUILD.md §6 후보). **📍 남은 순서: ① NSIS 설치본 로컬 실측(BUILD.md §4 체크리스트 — userData config 기록·설치본 내 settings.json 부재·§58~§64 기능) ② main 병합 → repo public 전환(사용자) ③ public 후 draft 릴리스 publish(`gh release edit v0.1.0 --draft=false`) — 그때부터 설치본 자동업데이트 동작 ④ 정식 v1.0 전 버전 범프 결정.**) (§83 신규 — **공개 직전 방어 정비 1차: 시크릿/이력/PII 전수 스캔(임계경로 ③ 전반부)**: repo public 전환 전 안전성 확정 목적, 233 커밋 전수. **결과 = 이력 클린, 히스토리 재작성 불필요**: ① 실 API 키 이력 0건 — 현재 키 프리픽스(sk-c61) `-S` 검색 0, AIza/ghp_/github_pat_ 히트는 전부 3ff3e4a(ADR0003)의 **감지기 정규식+더미 픽스처** ② 이력상 settings.json 블럽(구 pyside6_port, f759ebb 에서 삭제)도 `api_key:""` 빈값 확인 ③ postgresql:// 전부 플레이스홀더/코드(실 DSN 무) ④ `.env` 실파일 커밋 이력 무(.env.example 만) ⑤ root config/settings.json 은 최초부터 gitignore, data/·tmp/ 미추적. **조치(스테이징만, 미커밋)**: `git rm -r --cached tests/results` 279개(json 209 + **데스크톱 스크린샷 png 70**) — .gitignore 52줄에 기선언됐으나 ignore 이전 커밋분이 추적 잔존하던 것 이행(디스크 보존). 로컬경로(doosung) 노출 62파일 중 52개가 이걸로 해소. **사용자 결정 5건(같은 세션 §83b 에서 즉시 확정·이행)**: ⓐ ROADMAP §0 Owner 옛 핸들/이메일 → `@oddsung` 교체+이메일 제거 ✅ ⓑ saas CHANGELOG 의 개인 이메일 → "테스트 계정"으로 마스킹 ✅ ⓒ **회사 흔적 전부 제거(사용자: "개인 프로젝트, 회사 관련 내용 불필요")** — 회사 도메인/명칭 11곳(base_adapter 주석 예시→example.com, workflow_engine 주석, test_core §78g 픽스처 창제목→"사내 그룹웨어", test_scenarios docstring, handoff §5 표 5곳·§78g recap) 일반화 ✅ (이력에는 잔존 — 저민감, BFG 재작성은 전 클론 무효화라 비권장·미실시) ⓓ 원격 stale 브랜치 `claude/market-analysis-djqYu`(시장/경쟁 분석 1커밋) → **MARKET_ANALYSIS.md 를 `docs_private/`(신규 gitignore) 로 보관 후 원격 브랜치 삭제** ✅ ⓔ 내부 문서 = **전략 문서만 분리** — `docs/commercial_review.md` → `docs_private/` 이동(추적 해제, 이력 잔존 수용), handoff/ROADMAP/saas 설계 문서는 공개 유지 ✅. `.gitignore` 에 `docs_private/`+`tmp/` 추가(후자는 미추적이지만 실수 커밋 방지). **참조 정리**: README(en/ko)·CONTRIBUTING(en/ko) 의 commercial_review 실링크/언급 6곳 제거·재서술(Phase 2 게이트는 "내부 go/no-go 기준"으로), ROADMAP·ADR0003·handoff 의 md 링크는 "(docs_private/ 비공개 보관)" 평문화. ADR 0001~0004 결정자 @toytiger→@oddsung 통일. 원격 브랜치 삭제 **실행 완료**(`git push origin --delete`, 문서는 docs_private/MARKET_ANALYSIS.md 보관). **검증**: 추적 파일 전수 — 회사 도메인/명칭·개인 이메일·옛 핸들 흔적 **0건**, core 243/243 + scenarios 73/73 + ruff 그린(§78g test_244 픽스처 창제목 일반화 후에도 그린). **📍 다음: 세션 작업 커밋 → repo public 전환(사용자 GitHub 액션) → ④ NSIS 실측(재빌드: §81 spec + §82 포함) → ⑤ v1.0 릴리스(`dist:publish`)+발표.**) (§82 신규 — **electron-updater 자동 업데이트 도입(임계경로 ②, desktop_v3 전용·core 0줄)**: §81 결정(NSIS+자동업데이트) 이행. ① package.json — `electron-updater` 의존성(runtime deps, externalizeDepsPlugin 이 main 번들서 제외→electron-builder 가 node_modules 동봉) + `build.publish` = github oddsung/ohdo + `dist` 에 `--publish never` 명시(로컬 빌드가 릴리스 업로드 안 하게) + 신규 `dist:publish`(GH_TOKEN 으로 setup.exe/latest.yml/blockmap 업로드). ② main/index.ts `setupAutoUpdater()` — `app.isPackaged` 가드(dev 는 app-update.yml 없음), autoDownload + autoInstallOnAppQuit(배너 무시해도 종료 시 설치), 시작 10s 후 + 4h 간격 check, `update-downloaded` → renderer `updater:event` push, `updater:install` IPC → `quitAndInstall`(before-quit 의 브리지 SIGTERM 체인 그대로 탐), error 는 콘솔만(repo private 동안 404 소음 방지). ③ preload `onUpdaterEvent`(구독+해제 반환)/`installUpdate` + env.d.ts 타입 동기화. ④ 신규 `UpdateNotice.tsx` — 하단 중앙 배너("새 버전 준비" + 재시작/나중에), App.tsx 렌더, i18n `updater.*` ko/en(en 은 Catalog 타입이 키 패리티 강제). **미서명(§81)이라도 electron-updater 는 실행 앱이 미서명이면 서명 검증 skip → 동작**. 실동작 전제: repo public + 첫 GitHub Release(§81 ③번 순서와 맞물림) — 그 전까진 check 가 조용히 실패(무해). test_248 +1(배선 sentinel: publish 설정/이벤트 체인/배너/i18n — core **243/243**), tsc·electron-vite build·ruff 그린. BUILD.md §4 실측(구버전→배너→재시작 업그레이드) + §5 갱신, ROADMAP 임계경로 ② 체크. **📍 다음: ③ 공개 직전 방어 정비(시크릿/이력 점검)→repo public 전환 ④ NSIS 설치본 실측(사용자 머신 — 재빌드 필요: freeze 는 §81 spec 반영, Electron 은 §82 포함 `npm run dist`) ⑤ v1.0 릴리스(`dist:publish`)+발표.**) (§81 신규 — **배포 재결정: v1.0 출시 형태 확정(사용자 결정 4축)**: §80 지정 과제 수행. 사전 확인 — ROADMAP·§64·§3.0 검토 + repo 실상태(github.com/oddsung/ohdo **아직 private·Stars 0** — 5/12 게이트 평가 때와 동일, 유통 작업 전부 미착수). 결정: ① **전략 큰 틀 유지** — desktop_v3 OSS flagship·v1.0 출시 집중·SaaS parked(2026-06-10 결정 재확인) ② **repo 정리 후 조기 public 공개** — 공개(star 측정·평판 축적 시작점)와 발표(Show HN/Reddit — v1.0 릴리스에 맞춤)를 분리, 공개 직전 방어 정비(시크릿/이력 점검) 선행 ③ **v1.0 미서명 출시** — SmartScreen 경고는 README 안내로 감수(개발자 타깃 OSS 초기 관행), 서명은 사용자 확보 후 SignPath OSS(무료·심사, 공개 repo 전제)/Azure Trusted Signing(월 ~$10, 개인 온보딩 제한 이력 확인 필요)/OV(연 $100~400) 재평가 ④ **배포 스코프 = NSIS + electron-updater 자동업데이트**(GitHub Releases provider, 미서명 동작) — 초기 사용자에게 수정사항 밀어넣는 피드백 루프 확보, portable ZIP/winget 은 v1.0 이후. config 영속성(`--config-dir`+first-run 복사)은 실질 버그로 스코프 무관 선행 확정. **ROADMAP 동기화 완료**(§0 날짜 + §3.0 배포 재결정 반영 임계경로 재구성 + §10 변경 로그 — CLAUDE.md 규칙 준수). **§81b 같은 세션 — 임계경로 ① config 영속성(`--config-dir`) 구현 완료(core 0줄)**: 조사 중 **spec 키 유출 벡터 발견** — `ohdo-bridge.spec` 이 `config/` 를 datas 로 통째 동봉해 빌드 머신의 `config/settings.json`(**DeepSeek API 키 실값 포함** — gitignore 라 repo 는 안전)이 설치본 `_internal/config` 에 들어가던 문제(§64 실빌드 산출물이 이미 해당 — 미배포라 무해). fix ①(spec): 명시 파일 동봉으로 교체 — `default_settings.json`+`prompts.json` 만, `settings.json`/`prompts_archive` 제외 + 통째 동봉 금지 주석. fix ②(브리지): `deps.settings_path(app)`(app.state.config_dir 기반 런타임 해석) + `seed_config_dir`(mkdir + settings.json 미존재 시 1회 이관, 기존 파일 절대 미덮음) 신규, `create_app(config_dir=)` 파라미터, settings.json 읽기/쓰기 4곳(deps.get_app_service / routes settings GET·PUT / ai 엔진 persist / sessions export)을 settings_path 경유로 전환 — ai.py/settings.py 의 **import-time `_SETTINGS_PATH` 상수 제거**. `default_settings.json`/`prompts.json` 은 항상 번들(CONFIG_DIR)에서 읽음 — 사용자 쪽 구버전 사본(stale prompts) 방지, 앱 업데이트가 새 가이드(#24 등)를 그대로 전달. fix ③(/environment): config_dir 지정 시 `EnvironmentScanner(config_dir=)`(기존 core 공개 파라미터)로 스캔 캐시(environment.json)도 사용자 쪽 기록 — 번들 내부 쓰기 금지(perMachine 대비). fix ④(배선): `__main__ --config-dir` 인자 + Electron packaged spawn `--config-dir <userData>/config`(index.ts, dev 는 미지정 → 프로젝트 config/ 로 v2 와 공유 유지). 신규 설치 first-run 은 settings.json 없이 시작 — GET /settings 의 defaults 병합으로 안전(현 fresh-clone dev 와 동일 동작, AI 는 온보딩/설정에서 구성). test_247 +1(seed 밀폐/미덮음 + PUT 이 config_dir 에만 기록·프로젝트 config 불변 + 하위호환 기본경로 + spec 유출 sentinel + 배선 sentinel) — core **242/242** + scenarios 73/73 + recording_fixtures 2/2 + ruff·tsc·electron-vite build 그린. BUILD.md 갱신(§4 실측 2항목 추가 — userData config 기록 확인 + **설치본 `_internal/config` 에 settings.json 부재 확인(있으면 배포 금지)**, §5 config 한계 해결 표기 + 서명/업데이터에 §81 결정 반영). **📍 다음: 임계경로 ② electron-updater 도입 ③ 공개 직전 방어 정비→repo public 전환 ④ NSIS 설치본 실측(사용자 머신 — BUILD.md §4 신규 항목 포함) ⑤ v1.0 릴리스+발표.**) (§80 신규 — **IME shim 한/영 키 복원(core, 사용자 지시)**: 사용자 실측 — 자동화 후 한/영 키가 죽고 Win+Space 로만 복귀. 원인 = hotkey 래퍼의 `force_english_ime` 가 `LoadKeyboardLayoutW("00000409")`+`WM_INPUTLANGCHANGEREQUEST` 로 **입력 언어를 통째로 전환하고 복원 안 함**(US 레이아웃에선 한/영 키 무동작) + US 레이아웃을 입력 언어 목록에 영구 추가. fix(core/workflow_engine `_IME_COMPAT_SHIM` 재작성): ① 강제 전 창별 원래 HKL+IME 변환모드 저장(최초 1회만) ② **디바운스(2.5s) 자동 복원** — 마지막 자동화 입력 후 원래 레이아웃/모드 복원(threading.Timer, 커널 상주) ③ 시스템에 이미 있는 영문 레이아웃 재사용(신규 로드 회피), 신규 로드했으면 복원 시 `UnloadKeyboardLayout` 로 목록 원복 ④ 이미 영문이면 레이아웃 전환 생략 ⑤ `pyautogui.write/typewrite` 도 래핑(ASCII 입력이 직전 hotkey 강제에 우연히 의존하던 것 명시 보장). Save As 신뢰성(§닫힌루프 원목적)은 그대로 — 강제 자체는 유지, 복원만 추가. **실측 프로브(scratchpad ime_restore_probe.py): 메모장 HKL 0x0412→강제 0x0409→2.5s 후 0x0412 복원 + 입력 언어 목록 불변 확인.** test_246 +1(core **241/241** + scenarios 73/73 — v2 공유 무회귀), ruff 그린. 브리지 재시작 + **커널 재시작(세션 첫 실행 시 새 라이브러리 블럭 주입) 필요** — 기존 커널엔 옛 shim 이 남아있을 수 있어 앱 재시작 권장. **사용자 실사용 확인 완료(한/영 키 정상 복원)**. **다음 세션 지정(→ §81 에서 수행 완료): "배포"를 새로 결정하는 것부터 시작** — 기존 결정 참고 자료: ① `docs/ROADMAP.md` — 오픈코어 전략·글로벌+한국 dual-locale·Phase 계획(2026-06-10 결정: desktop_v3 = OSS flagship, v1.0 출시 집중, SaaS MVP parked) ② §64 Phase E 배포 freeze — PyInstaller 브리지+electron-builder 파이프라인 그린, `docs/BUILD.md` 런북, 잔여=NSIS 설치본 GUI 실측·코드서명·config 영속성(`--config-dir`) ③ 출시 후 유통 구상 — 공개 repo·영어 Show HN/Reddit·초기 사용자 5명(§3.0). 새 결정 시 ROADMAP 동기화 규칙(CLAUDE.md) 준수.) (§79 신규 — **실행 중 시각 효과(run FX) 풀 패키지**(사용자 요청+범위 선택): 전체/단일 실행 동안 ① 디스플레이별 **테두리 글로우**(§77 클릭통과 오버레이 패턴, 파랑 펄스→종료 시 성공 초록/실패 빨강 플래시, 수동중지는 무플래시) ② 상단 **상태 HUD**(주 디스플레이, "ohdo 자동화 실행 중 · k/N 완료") ③ **커서 링**(Electron 커서 폴링, 커서 위치 디스플레이만) ④ **클릭 리플**(신규 `api_server/fx_pump.py` **관찰 전용** LL 훅 — pick_pump 구조지만 클릭 절대 미점유(항상 CallNextHookEx), §49 fix9 argtypes 준수; `routes/fx.py` POST /fx/start·stop + GET /fx/clicks?since= 폴링 계약; main 이 120ms 폴링→물리픽셀→디스플레이 로컬 DIP 변환→오버레이 runfx:poll 소비) ⑤ **작업표시줄 진행바**(setProgressBar). 신규 renderer 엔트리 run_overlay.html/ts(vite 4번째), preload runFxStart/Progress/Stop+ohdoRunFx.poll, useExecution 이 시작/step 완료/종료에 배선(FX 실패는 실행에 무영향 best-effort). core 0줄. test_245(+1, 버퍼 since 계약/ring 상한/라우트 — 훅 arming 은 테스트 미설치 정책 §230 동일). **Playwright+실클릭 스모크 실측**(runfx_check.mts): HUD/커서링/종료 초록 플래시 스크린샷 확인 + /fx/clicks 실클릭 1건 캡처(리플은 캡처 타이밍상 미촬영 — 동일 데이터 경로). 잔여: running 테두리 펄스가 4K 에서 다소 미묘(두께/불투명도 취향 조정 후보), 생성코드가 실행 중 스크린샷 찍으면 오버레이가 함께 찍힘(§70식 숨김 후속 후보). **§79 후속(8차 실측 — "메모장은 떴는데 스텝은 TimeoutError")**: 세션 bbbbba89 step2(메모장 idempotent 실행) — 실행 블록 깨끗(삭제-재생성 잔재 아님) + **FX 간섭도 실측 기각**(fx_interference_probe.mts: FX armed 상태서 동일 스니펫 1.7s 성공; FX 없인 0.2s). 진범 = **Win11 스토어 메모장 콜드스타트**가 코드의 1.5s sleep+connect timeout=10 을 초과(AppX 활성화 10s+ 가능) → 창은 늦게 뜨고 스텝은 이미 실패. fix: prompts.json **가이드 #24**(새로 실행한 앱의 재연결 connect 는 `timeout=30` — 성공 시 비용 0, 재사용 시도의 첫 connect 는 timeout=5 유지) + test_237 확장(core **240/240**). ⚠️ prompts.json 편집은 반드시 스크립트(json.load→append→dump)로 — bash 인라인 `python -c` 는 백틱이 command substitution 으로 먹혀 텍스트 유실(실제 발생, 재작성으로 복구). 기존 step 은 재생성 또는 코드 편집(timeout=10→30)으로 갱신.) (§78 신규 — **agy 1.1.8 브레이크→DeepSeek 전환 + IDE 타이틀바 개편 + §76/§77 멀티모니터 실측 통과**: ① 사용자 보고 "empty prompt / nonexistent pipe" 진단 — agy(Antigravity CLI) 1.1.8 이 `type f | agy -p ""`(§36 ConPTY 우회)의 빈 `-p` 시 stdin 파이프를 안 읽고 즉시 종료(구버전 동작 회귀). 추가로 올바른 `-p "프롬프트"` 도 파이프/ConPTY 환경서 "timeout waiting for response"(`--log-file` 실측: 인증·메시지 전송 OK, 대화 IDLE→RUNNING 미전환, printed=0; `agy models` 4s 정상=백엔드 무관; IDE 통합터미널 env 로 "local chrome mode" 오인 로그도 관찰) — 다중 원인이라 agy 보류, **`config/settings.json ai.selected=openai_compat`(DeepSeek, 기등록) 전환**(사용자 결정). ② **타이틀바+메뉴바 개편**(사용자 결정: VS Code/Cursor 통일감+OpenCode 메뉴구조 참고): `titleBarStyle:"hidden"`+`titleBarOverlay`(WCO 캡션 버튼, h=36, 테마 토글 시 `window:set-titlebar-theme` IPC 로 색 동기) + 신규 `TitleBar.tsx`(oh로고=홈 · 햄버거 드롭다운=새세션/환경점검/시크릿/설정/언어/테마/시작안내 · 새세션 퀵버튼 · TabBar 임베드 · 우측 설정/테마) + **ServerRail 삭제**(§59 유틸 상단 이전) + 창 기본 1440×900·마지막 bounds/최대화 `userData/window-state.json` 영속(디스플레이 밖 가드). desktop_v3 전용(core/api_server 0줄), tsc/build 그린, Playwright 스모크(devloop/scenarios/titlebar_check.mts) 실측. ③ §76/§77 을 사용자 3모니터(150/175/100%)에서 자동 실측 — `devloop/scenarios/multimon_hover.mts`+`multimon_target.py`(신규): hover 붉은 박스 보조모니터 num7 정확·타 디스플레이 미표시, 픽 `[Button] "7"` 정확, §70 캡처 무테두리. ④ **pick element_context 창제목 유실 실버그 fix**(사용자 실측 "버튼 클릭 코드 실행 오류", 세션 66c61ea1): `capture_element_at`(recorder 형식, `window_title` 키)의 메타를 `_build_element_context`(pick.py)가 rect 만 정규화한 채 `get_element_info_text`(picker 형식, `parent_window_title` 키)에 넘겨 **창제목이 항상 유실** → 템플릿 connect 라인이 `connect(title="...")` 플레이스홀더(win_inspector.py 863) → 템플릿을 충실히 복사하는 DeepSeek 이 그대로 출력 → 실행 시 "..." 창 못 찾고 실패. (agy 는 템플릿을 무시하는 버릇(§68/§71) 탓에 이 버그가 가려져 있었음.) fix(api_server only, core 0줄): v2 PR-19d `_recorder_meta_to_picker_dict` 동등 매핑(parent_window_title/is_browser(exe)/screen_x·y/backend)을 `_build_element_context` 에 추가(test_240). **§78b 후속(같은 날 사용자 2차 실측)**: 매핑 후에도 실패 — core `_resolve_top_window_title`(element_inspect)가 UIA 트리를 데스크톱 루트까지 올라가 **마지막 비어있지 않은 이름 = Win11 가상 데스크톱 이름("데스크톱 1")** 을 반환 → `connect(title="데스크톱 1")` = 0 windows → TimeoutError. fix(api_server only, core 0줄): pick.py 에 `_derive_root_window_title`(요소 hwnd→`GetAncestor(GA_ROOT)`, 없으면 rect 중심 `WindowFromPoint`; GA_ROOT 는 top-level 에서 멈춰 데스크톱 이름 불가) **우선** + `_looks_like_desktop_root` 필터(데스크톱류 이름은 폴백으로도 금지). ⚠️ core `_resolve_top_window_title` 자체는 미수정(불변 원칙) — **recorder 의 element_meta.window_title 도 같은 오염 가능**(recorder codegen 은 process_id 우선(PR-19e)이라 완화되나 core fix 는 사용자 승인 대기 후보). 테스트 함정: 가짜 rect 를 주면 `WindowFromPoint` 가 실제 화면의 그 좌표 창을 잡아 비결정 — 폴백 검증은 rect=None 으로(밀폐), 실 재도출은 Progman("Program Manager")으로. **§78c 후속(3차 실측 — Word 리본 탭 클릭 STEP1 은 §78a/b 로 성공✅)**: STEP2(Chrome 탭 픽)에서 DeepSeek 이 "## 선택된 UI 요소" 코드 템플릿을 **"이미 실행된 코드"로 오독** → 새 스텝 블록에 주석만 생성("템플릿의 클릭 코드로 충분") + 이전 스텝 코드도 응답에서 누락 → step_code="" · 누적 generated_code 3745→251자 축소(세션 오염). fix 2겹: ① pick.py directive 강화("템플릿은 아직 실행 안 됨 — 새 블록 안에 복사 필수, 주석만 금지, 이전 스텝 유지") ② **결정적 가드 신규 `api_server/step_guard.py`**(사용자 선택, §72 철학): 생성 후 (가드1) step 에 실행문이 없으면(델타 빈/마커블록 주석뿐) element_context 의 ```python 템플릿 코드로 step_code 대체 (가드2) 이전 step 마커가 누적 코드에서 사라졌으면 §73 `_rebuild_generated_code_chain` 으로 복원. generate.py WS 가 §72 교정 직후 `apply_step_guards` 호출. 통합 검증(scratchpad guard_integration.py): 사용자 시나리오 재현 → step_code=connect+_resolve_element+클릭 · step1 코드 복원 확인. test_242 +1(core **237/237**), ruff 그린. 브리지 재시작 후 STEP2 재생성 필요. **§78d 후속(4차 실측, 07-29 — 첫 core 수정 2호, 사용자 승인)**: Gmail 탭 픽 → `connect(title="받은편지함 (17, found_index=0) - ... - Chrome")` 오염 → 0 windows. 원인 = `core/adapters/base_adapter.py restore_user_strings` 8단계(5/5 G-시리즈 "ambiguous 방지")의 found_index 자동추가 정규식 `[^)]*` 가 **title 문자열 안의 ')'** — Gmail "받은편지함 (17)" — 에서 끊겨 문자열 한복판에 `, found_index=0` 삽입(기보유 검사도 잘린 매치라 무효). 괄호 없는 제목(메모장/계산기/Word)만 테스트돼 잠복. fix: 인자 패턴을 문자열 인지 `(?:"[^"]*"|'[^']*'|[^()])*` 로 교체 + title 존재 검사를 replacement 함수로 이동(기존 동작 — 미보유 추가/보유 유지/title 없음 무시 — 보존, v2 공유 자동 수혜). test_243 +1(core **238/238**) + recording_fixtures 2/2 + ruff 그린. **주의: prompt_quality 스위트에 기존 실패 4건 존재(test_10/21/30/31 — element context 포맷 기대 불일치, 오늘 변경·엔진 전환과 무관 확인) — 별도 백로그.** 잔여 리스크: 브라우저 full-title connect 는 실행 시점 활성 탭/안읽음 수 변화에 취약(§ browser 브랜치 설계) — 재발 시 브라우저 connect 전략(title_re 안정 세그먼트/process 기반) 개선 후보. **§78e(5차 실측 — "스텝 전부 삭제 후 재생성했는데 옛 Word 코드가 보인다")**: 저장소는 전수 스캔상 깨끗(삭제 경로 프롬프트 유출도 시뮬레이션으로 CLEAN 확인) — 원인은 **CodeViewer 의 Monaco `path={'step-'+stepId}` stale-model 버그**. Monaco 모델은 전역·영속이라 스텝 전부 삭제(에디터 언마운트, 모델 잔존) 후 재생성된 **같은 step_id** 가 옛 모델을 재사용하는데 @monaco-editor/react 는 `value` 를 모델 **생성 시에만** 적용 → 삭제된 스텝(Word)의 코드가 그대로 표시(다른 세션의 같은 id 도 충돌). fix: `path` prop 제거(단일 모델, value 가 항상 지배). desktop_v3 전용, tsc/build 그린. 재빌드+앱 재시작 필요. **§78f(6차 실측 — 브라우저 탭 2스텝: step1 "오늘" 링크 클릭 성공✅/step2 "메일" 탭 실패)**: DeepSeek 이 step2 블록을 **부분 요약** — try:/pyautogui.click 등 실제 문장을 설명 주석("# pyautogui 클릭 (브라우저 탭...)")으로 대체하고 본문 print 는 들여쓰기 그대로 잔존 + 시작 마커 누락 + center_x 재계산 생략 → dangling indent(SyntaxError) + 클릭 문장 자체가 없는 코드. §78c 가드1은 "실행문 전무"만 검사해 미발동(실행문 일부 존재). fix: `is_broken_step_code`(실행문 전무 **또는 ast 컴파일 불가**) 로 가드1 트리거 확장 — step_code 는 jupyter 단독 실행 단위라 컴파일 가능이 계약. + pick directive 에 "코드 요약/문장의 주석 대체 금지, 블록은 완전·컴파일 가능해야" 추가. test_242 (4) 확장(실측 파편 감지, core **238/238**), ruff 그린. 브리지 재시작 + step2 재생성 필요. **§78g(7차 실측, 07-31 — 예고된 브라우저 full-title 취약성 재현)**: 세션 38a2d012 — step2(그룹웨어→스카이스캐너 탭 클릭)가 1차 전체실행 성공 → **탭 전환으로 Chrome 창제목이 바뀜** → step3 생성 후 2차 전체실행 시 step2 의 `connect(title="홈 - 사내 그룹웨어 - Chrome")` 가 0 windows(성공한 실행이 스스로 전제를 파괴하는 구조 — "한 번 성공 후 재실행 실패"의 원인). fix(pick.py `_stabilize_browser_connect`, core 0줄): is_browser 요소의 템플릿 connect 를 **process 우선(pick 시점 PID, 재시작 전까지 제목 무관) + 프로그램명 title_re 폴백(`.*Chrome`, PID 만료 대응)** + `win = app.top_window()` 로 교체(컴파일 검증, 실패 시 원본 유지; 데스크톱 요소 불변). test_244 +1(core **239/239**), ruff 그린. 브리지 재시작 + 기존 브라우저 스텝(2·3) 재생성 필요. 관찰: step3 셀렉터 `title="￼"`(U+FFFC — 검색창 name 깨짐) — 재생성 후 재관찰 대상. **재생성 필요**: 기존 깨진 step 은 브리지 재시작 후 요소 재선택+재요청. element_meta 가 v3 step 에 미영속(재생성 시 요소정보 없음)인 갭은 별도 후속. **남은: §60 영역캡처 오버레이 멀티모니터 동종(§77 패턴 미적용) + v3 step element_meta 영속(재생성 대비) + agy 복구 시 stdin 대안 프로브(scratchpad agy_probe*.py) + 사용자 앱 재시작 후 DeepSeek 생성 확인.**) (§77 신규 — **멀티모니터 hover 붉은 박스 = 디스플레이별 오버레이**: §76 후속 — 보조모니터 요소 선택은 되나(좌표 OK) hover 붉은 박스가 주모니터에서만 보임. 원인: picker 오버레이가 `transparent:true` 단일창으로 가상 데스크톱 전체를 덮는데 **Windows 에서 투명창은 주 디스플레이에만 합성**(보조모니터 확장영역 렌더 안 됨, 특히 다른 DPI). fix: `screen.getAllDisplays()` 마다 오버레이 창 1개(각 단일 DPI→정상 렌더), `pick:hover` 가 `fromWebContents` 로 폴링 창 식별→요소가 그 디스플레이일 때만 box 반환, 모든 HWND 를 리스트로 z-order/§70 숨김 등록(`pick_pump._overlay_hwnds` 리스트화, `/pick/overlay` hwnds 수용). desktop_v3+api_server, core 0줄. test_239 +1(core **234/234**), tsc/build/ruff 그린. **남은 동종: §60 영역캡처 오버레이도 단일 spanning 투명창(미수정).** 사용자 재빌드+브리지 재시작 후 보조모니터 hover 박스 확인.) (§76 신규 — **멀티모니터(다른 DPI) 요소 선택 회귀 fix**: 모니터 2대+ 각 해상도(DPI) 다를 때 요소 선택 어긋남(v1 해결→v3 재발). 분석: overlay 변환수학은 정상(`screenToDipPoint` 모니터별 정확)이나 **Python 브리지 프로세스가 PER_MONITOR_AWARE_V2 미설정** — v1 은 `main.py` 에서 설정했으나 v3 브리지는 Electron 이 spawn 하는 별도 프로세스라 누락(`pick_pump` 자체 LL 후크라 `get_hook_manager` 의 자동 트리거도 안 탐) → DPI-unaware 면 `GetCursorPos`/LL-hook/`ElementFromPoint` 좌표가 OS 가상화→보조(다른 DPI)모니터 오선택+박스 어긋남. fix(`api_server.__main__`, core 0줄): `main()` 첫 동작으로 `ensure_dpi_awareness()`(core 공개) 호출 → 모든 pick·hover·캡처 좌표가 물리픽셀로 일관(§66~70 스크린샷도 동반 정상화). test_238 +1(core **233/233**), ruff 그린. **사용자: 브리지 재시작 후 보조모니터 재확인.**) (§75 신규 — **메인 창 stuck-topmost fix**: 전체 실행 후 ohdo 창이 최상위에 박혀 다른 창이 앞으로 못 오던 문제. 원인: `window:focus` IPC 가 `setAlwaysOnTop(true)→focus()→(false)` 를 한 tick 동기 실행 → 일부 Windows 에서 WS_EX_TOPMOST 안 풀림. fix: topmost 해제를 `setTimeout(250ms)` 로 지연(main/index.ts, 프론트만, tsc/build 그린). step 삭제 후 입력불가(②)는 네이티브 window.confirm 의심 — ① 수정 후 재확인 요청(연관 시 동시 해소 가능).) (§74 신규 — **셀렉터 교정 변수명 일반화**: 04:48 세션 탭닫기 O / 06:52 X 비교. 원인: §72 교정 regex 가 LHS 변수명 `element` 만 매칭 — 06:52 은 AI 가 `element_click = win.child_window(control_type="Document")` 라 불일치→교정 안 됨→Document 클릭. fix(selector_enforce.py, core 0줄): regex LHS 를 `\w+` 로 일반화(어떤 변수명도 매칭, win.child_window=element 해석이라 안전). test_232 확장(core **228/228**). 사용자 브리지 재시작 후 06:52 step4 재생성 → 탭 닫힘.) (§73 신규 — **코드 뷰어 step 별 표시**: 사용자 — v3 가 step 선택 시 누적 generated_code(이전 step 전부+import)를 다 보여줌(v2 는 step 블록만). fix: App.tsx 가 `step.step_code`(델타=그 step 블록) 표시(폴백 generated_code). 편집도 step_code 단위 — PUT update_step 이 step_code 저장 후 `_rebuild_generated_code_chain`(library+step_code 누적, reorder/delete 패턴)으로 누적코드 재구성→편집이 후속 step 에 전파(이전엔 편집블록으로 generated_code 를 덮어 후속 delta 깨짐). core 0줄(공개 API). test_233 +1(core **228/228**), tsc/build 그린. 사용자 브리지 재시작+재빌드 후 확인.) (§72 신규 — **picker 요소 셀렉터 결정적 강제 = 탭닫기 오클릭 진짜 해결**: 사용자 반론(엔진 교체 불필요, v2 검증됨, step2≡step4)이 정당. 재조사로 **엔진 무능 아님** 확정 — 동일 세션 STEP2("+") 는 `auto_id="AddButton"` 정확 사용·성공, STEP4("x") 는 `control_type="Document"`·실패. 차이는 프롬프트 신호-잡음: STEP4 프롬프트에 Document 31회 vs CloseButton 1회(직전 step 들이 Document 클릭+manually_edited "반드시 유지" → AI 가 다수 패턴 추종). fix(사용자 선택=결정적 셀렉터, api_server only/core 0줄): `selector_enforce.py` 가 생성 후 그 step 의 `element=win.child_window(...)` 를 픽 요소 auto_id/title 로 교정(마커영역 한정, guard·idempotent), `generate.py` WS 가 호출+step_code 재추출. test_232 +1(core **227/227**), 프론트 무변경. **사용자: 브리지 재시작 후 STEP4 재생성 → 탭 닫힘.**) (§71 신규 — **step delta 추출 dangling-block 버그 fix(core, 첫 core 변경·사용자 승인) + 탭닫기 미동작=AI 타겟팅**: 세션 ba7f4dc8 탭닫기 미동작 조사. ① **core 버그**: `extract_code_delta` 가 prev 중복 라인 제거 시 제어 헤더는 남기고 본문만 제거 → 연속 클릭 step 의 walk-up/click 본문이 동일해 dangling block → SyntaxError(저장 step_code 깨짐). fix: block-aware(유지된 헤더 블록 본문은 보존, module-level 중복만 제거). test_231 +1(core **226/226**), v2 공유 무회귀. **단 이게 run-time 원인은 아님** — run-time `extract_step_delta_code` 는 마커 추출+컴파일검증으로 깨진 step_code 우회. ② **실제 탭닫기 미동작 원인**: AI 가 picker 로 고른 "x"(이름=탭닫기, auto_id=CloseButton) + §68 지시 완벽 전달에도 무시하고 `control_type="Document"` 클릭 코드 생성 → 텍스트영역 클릭, 탭 안 닫힘 = **엔진 준수 한계(미해결)**. 엔진 교체/수동편집/결정적 codegen 논의 필요. **브리지 재시작 필요.**) (§70 신규 — **캡처에 picker 붉은 오버레이 찍힘**: §69 후 이미지는 보이나 "+" 캡처에 빨간 테두리. 원인은 §66 타이밍 — `_capture_element_image` 의 mss grab 시점에 Electron 붉은 박스 오버레이가 아직 안 닫힘(렌더러가 /pick/click 응답 후 닫음). fix(api_server only, core 0줄): pick_pump 가 `get_overlay_hwnd()` 노출(finally 클리어 제거) + `routes/pick._hide_pick_overlay()` 가 grab 직전 오버레이를 `ShowWindow(SW_HIDE)`(cross-process)+0.12s 재합성 대기 후 grab(복원 안 함, 미등록 graceful). test_230 +1(core **225/225**), ruff 그린, 프론트 무변경. **사용자: Python 브리지 재시작 후 재픽 — 빨간 테두리 없이 "+"만 찍히는지 확인.**) (§69 신규 — **캡처 이미지 "깨진 표시" = CSP img-src 에 blob: 누락**: §68 재빌드 후 썸네일 박스는 정상이나 이미지가 broken-image 아이콘. 원인은 순수 CSP — `index.html` 의 `img-src 'self' data:` 에 blob: 가 없어 CaptureThumb 의 object URL(`blob:`) `<img>` 가 차단(서빙 엔드포인트·CORS·파일은 TestClient 로 정상 검증). fix: `img-src 'self' data: blob:` 한 줄(코드 무수정). build 그린·번들 반영 확인, core 224/224 불변. **사용자 재빌드/새로고침 후 "+" 이미지 표시 확인**. P1 오클릭은 §68 AI 준수 이슈로 별개.) (§68 신규 — **캡처 썸네일 고정크기 + 셀렉터 단순화 방지**(§67 재빌드 후 사용자 실측 ba7f4dc8): ① 캡처 이미지가 "검은 세로줄"로만 = `CaptureThumb` 너비 미고정 버그 → `h-20 w-28` 고정 박스 + img `object-contain fill`(작은 "+"도 업스케일·표시영역 일정). ② "+" 대신 엉뚱 Button 클릭 = **§66/§67 버그 아님** — 프롬프트 로그 실측상 §67 풍부한 컨텍스트(auto_id=AddButton + title 템플릿) 정상 주입됐으나 **AI(agy)가 템플릿 무시하고 generic `child_window(control_type="Button", found_index=0)` 로 단순화** → 첫 Button 오클릭(=AI 준수 문제, 근본은 prompts.json generic vs specific 템플릿 충돌=core/v2 공유라 사용자 결정 필요). v3-only 완화: `_build_element_context` 말미에 anti-pattern 금지 지시 추가(best-effort). test_229 확장(core **224/224**), tsc/build/ruff 그린, core 0줄. **사용자: 재빌드 후 재픽·재생성 — ②가 여전히 generic 이면 prompts.json 강화/엔진 논의.**) (§67 신규 — **pick race fix + 풍부한 element_context**: 사용자 실측(메모장 "+" pick→"클릭하기")에서 ① 요소 이미지 미표시 ② AI 가 "+" 무시하고 Document 클릭. **워크플로우 4에이전트로 단일 근본원인 규명**: `ChatPanel.send()` 가 `mutate()` 직후 동기 `clearPending()` 하는데 React Query 는 mutationFn 을 다음 microtask 에 실행 → mutationFn 이 pending 을 읽을 땐 이미 null → element_context=null(→AI 가 가이드 폴백 Document 복제) + imagePath=undefined(→attach 미호출, captures=[]). **Fix #1**: send() 가 mutate 시점에 pending/images 스냅샷 떠 mutate 인자로 전달(race 제거). **Fix #2(v2 동등)**: pick 이 한 줄 라벨 대신 core `get_element_info_text` 로 "## 선택된 UI 요소" 섹션+코드템플릿(가이드 #17 이 참조하는)을 element_context 로 반환(rect 리스트→dict 정규화+guard, core 0줄). test_229 +1(core **224/224**), tsc/build/ruff 그린. **사용자: 앱 재빌드/재시작(브리지 포함) 후 step2 재생성 필요.**) (§66 신규 — **step 카드 UX 3건**(사용자 보고): ① AI 설명 기본 숨김(step 선택 시만) ② 👤/🤖 컬러 이모지 → lucide User/Bot 단색 아이콘(Library/Initial 통일) [①② 커밋 e4dc28f] ③ **선택 요소 스크린샷 캡처+표시**: pick 은 원래 메타만 잡고 이미지 미캡처였음 → `/pick/click` 가 session_id 받으면 요소 rect 를 `capture_pump` 로 grab→세션 captures 저장(`image` 반환), 생성된 step 에 **표시 전용**(AI 무전송)으로 `POST .../steps/{id}/capture` attach, 새 `GET .../captures/{file}` 서빙(인증 fetch→blob), `CaptureThumb` 고정높이(h-20)+object-contain 으로 카드 높이 안정. §60 카메라 캡처도 같은 captures 라 함께 표시됨. core 0줄(`get_captures_dir`/`update_step` 공개 API 재사용), test_228 +1(core **223/223**), tsc/build/ruff 그린. 좌측 레일 폭 72→56px 축소(커밋 ff0db59)도 동일 세션.) (§65 신규 — **설정 다이얼로그 한글화**: 사용자 보고("설정 메뉴에 한글화 안 된 항목 많다") fix. §56 제네릭 설정 에디터가 `default_settings.json` 영문 원본 키를 라벨로 그대로 노출하던 것 → ko/en 카탈로그에 `settingsKeys` 맵(섹션+엔진+전 필드) 신규 + SettingsDialog 가 `labelFor(k)` 로 라벨 렌더(원본 키는 tooltip 보존, 미등록 키 graceful 폴백). desktop_v3 전용, core 0줄, tsc/build 그린.) (§64 신규 — **Phase E 배포 freeze 실측/de-risk**: §46 이 미룬 freeze 를 이 개발 환경(Windows, GUI 불가지만 freeze 는 CLI)에서 실제로 빌드·부팅·동봉까지 돌려 막힘 제거. core/PySide6/ui_v2 **0줄**. ① `pyinstaller>=6.0` 을 `build` extra 로 선언(문서의 `uv run pyinstaller` 가 미선언이라 실패하던 것) ② spec 죽은 hidden import `pywinpty`(배포명, import 모듈은 `winpty`) 제거 ③ **§63 기커밋 tsc 버그** 발견·수정 — `ChatPanel` 이 옛 `stopCommit` 구조분해(rename 누락), electron-vite=esbuild 가 타입 stripping 해 §63 "tsc 그린"이 오claim → `stopReview` 로 수정(녹화중단 버튼 런타임 throw 버그) ④ **packaged data-dir** = frozen 기본값이 번들 내부(`_internal/data`, 업데이트 시 세션 소실/쓰기불가 위험) → Electron `bridgeCommand` 가 `--data-dir %APPDATA%/ohdo/data` 전달(브리지 기지원, **TS만**) ⑤ `npm install` 로 §46 이 안 돌린 lockfile 동기화. **검증**: freeze exit 0 + frozen `ohdo-bridge.exe` 단독부팅 /health·/sessions·/environment(core 스캐너 frozen 실동작) + `electron-builder --dir` exit 0 동봉 확인(`resources/pybridge/ohdo-bridge.exe`) + tsc/electron-vite/core 222/222/ruff 그린. **신규 `docs/BUILD.md` 런북**(`.venv` 강조, freeze→복사→NSIS, winCodeSign symlink 트러블슈팅) + README 빌드 섹션. **남은: NSIS 설치본 GUI 실측(사용자 머신) + config 영속성(번들 내부 read, `--config-dir` 후속) + 코드서명**.) (§63 신규 — v3 패리티 백로그 P1 #22 녹화 review 다이얼로그: 녹화 종료를 "즉시 commit"에서 "검토 후 commit"으로. core 가 이미 stop_recording(변환된 step 반환)/commit_recording(세션 커밋)로 분리돼 있어 core 0줄. RecordingController stop("preview")(stop+변환, 커밋X)/commit_steps + POST /recording/stop_preview·/recording/commit 신규. 프론트 recordStore stopReview/commitReview/discardReview, RecordingReviewDialog(신규 — step 코드/설명 편집·삭제·순서변경 후 확정), Step 재구성은 알려진 필드만(메타 round-trip). test_227 +1(core 222/222). **#13/#19/#21/#22 모두 완료 — P1+ 백로그 소진.**) (§62 신규 — v3 패리티 백로그 P1 #21b 시크릿 @자동완성 + 평문 감지: ChatPanel 입력창을 SecretAutocomplete(신규)로 교체 — `@<prefix>` 입력 시 시크릿 라벨 드롭다운(↑↓/Enter/Tab/Esc) → 선택 시 `{{secret:label}}` placeholder 삽입(core 가 generate 시 vault 값 해결, 평문 노출 X). 전송 시 `POST /secrets/scan`(secrets_detector.detect 위임, **값 미노출**=미리보기만)로 평문 시크릿 감지 → 발견 시 마스킹 권고 confirm. core 0줄. test_226 확장(scan 라우트+무노출 가드). #21 완료. 남은: #22 녹화 review.) (§61 신규 — v3 패리티 백로그 P1 #21a 시크릿 볼트 CRUD: core ADR0003 `AppService.secrets_vault`(KeyringVault, OS keyring) 를 GET/POST/DELETE `/secrets` 로 노출(core 0줄). **값은 절대 renderer 로 반환 안 함**(label 목록만) — 실행 주입은 기존 push_secrets 배선 재사용. 신규 routes/secrets.py + SecretsDialog(레일 KeyRound 버튼 + 팔레트, label 패턴검증/마스킹입력/등록·삭제) + i18n secrets.*. test_226 +1(core 221/221, set→list→delete 라운드트립 + 값 미노출 가드). 남은: #21b @자동완성, #22 녹화 review.) (§60 신규 — v3 패리티 백로그 P1 #13 첨부 이미지(스크린 영역 캡처): 드래그 영역 캡처 오버레이(capture_overlay.html/ts, 클릭통과 아님) → main 이 DIP→물리픽셀 변환 → POST /capture/region → mss grab → 세션 captures 저장 → generate(images=[...])로 전달. core 0줄(generate_step images 파라미터 + 공개 get_captures_dir 재사용). 신규 api_server/capture_pump.py + routes/capture.py, GenerateRequest.images + WS/POST generate 배선, ChatPanel 카메라 버튼 + 이미지 chip. test_225 +1(core 220/220), tsc/build/ruff 그린.) (§59 신규 — 좌측 서버 레일(oh 로고 세로영역) 회수: 자리표시자였던 레일을 #2 "전역 네비/유틸 바"로 활용 — 로고=홈(세션 선택 해제), 하단에 도움말/환경점검/설정/언어/테마 아이콘(기존 사이드바 푸터에 밀집했던 것)을 이동. 사이드바 푸터는 브리지 상태(HealthDot)만 남김. 신규 `ServerRail.tsx`(App 인라인에서 승격), i18n `sidebar.home` 추가. 워크스페이스 도입 시 로고 아래 워크스페이스 아이콘 추가로 #1(프로젝트 전환기) 승격 예정 — 그래서 레일 제거 대신 Discord 레이아웃 유지. 순수 프론트(core/api_server 0줄), tsc/build 그린.) (§58 신규 — v3 패리티 백로그 P1 #19 온보딩 위저드: 첫 실행 시 자동으로 뜨는 4단계 위저드(환영+언어선택 / 환경점검 / AI엔진선택 / 완료+첫 세션 생성). 순수 프론트 — 환경점검(GET /environment)·엔진전환(GET·POST /ai/*)·세션생성은 모두 기존 엔드포인트 재사용, core/api_server 0줄. localStorage `ohdo.onboarded` 플래그로 1회만 자동, 사이드바 ? 버튼 + 팔레트 "시작 안내 보기"로 재오픈. uiStore onboardingOpen + i18n onboarding.* 카탈로그. tsc/build 그린(프로젝트 lint 스크립트 없음), core 219/219 유지.) (§57 신규 — v3 패리티 백로그 P1 #17 멀티 세션 탭: uiStore openTabs[] + selectSession 이 탭 자동 추가 + closeTab(활성 닫으면 인접 전환). TabBar 컴포넌트(채팅/코드 영역 위, 탭 전환/닫기). App 레이아웃 col 래핑. 세션 삭제 시 closeTab 으로 탭 정리. 순수 프론트, core 0줄. tsc/build 그린.) (§56 신규 — v3 패리티 백로그 P1 #20 설정 화면 확장: SettingsDialog 가 AI 엔진만 노출하던 것을 ai 외 모든 top-level 섹션(image/recognition/execution/ui/output_project/logging/element_picker/...)으로 확장 — 접이식 ConfigSection + path 기반 setByPath + 문자열배열 ArrayField(쉼표편집) + bool/num/str Field 재사용. 백엔드 GET/PUT /settings 는 §47 기구현(test_218), 순수 프론트 확장. tsc/build 그린.) (§55 신규 — v3 패리티 백로그 P1 #15 프로젝트 내보내기/가져오기: routes/sessions.py 에 POST /sessions/{id}/export(export_workflow, output_dir 하위 안전 폴더명+clobber 회피) + POST /sessions/import(import_workflow, session.json 가드). core 0줄. Electron main 에 fs:pick-directory(dialog)/fs:reveal(shell) IPC + preload pickDirectory/revealPath. 프론트 사이드바 import(헤더)/export(행) 버튼 + 팔레트 명령. test_224 +1 — export→import 라운드트립.) (§54 신규 — v3 패리티 백로그 P1 #18 환경 점검: routes/environment.py 신규 (GET /environment = core 공개 get_scanner().full_scan() 위임 — Python/패키지/CLI AI 상태, AppService 미경유). core 0줄. 프론트 EnvironmentDialog (사이드바 Activity 버튼 + 팔레트 명령, on-demand 스캔 + 로딩/재검사). test_223 +1 — 라우트+scanner 공개 API+get_system_info(full_scan 미호출).) (§53 신규 — v3 패리티 백로그 P1 #14 AI 엔진 퀵스위치: routes/ai.py 신규 (GET /ai/engines = list_ai_engines 위임, POST /ai/engine = switch_ai_engine 런타임 전환 + settings.json ai.selected 영속으로 설정 다이얼로그/재시작 일관). core 0줄. 프론트 EngineSwitcher 헤더 드롭다운(녹화중 숨김). test_222 +1 — read-only/실패 경로만(실 config 무변조).) (§52 신규 — v3 패리티 백로그 P1 #12 세션 복제: core 에 duplicate 메서드가 없어 api_server 가 공개 API 조합으로 구현 — POST /sessions/{id}/duplicate(create_session + steps/workflow_metadata 깊은 복사 + save_session, 캡처/스크립트 폴더 best-effort 복사). core 0줄. 프론트 사이드바 복제 버튼 + 커맨드 팔레트 명령(locale별 (copy)/(사본) 제목). test_221 +1.) (§51 신규 — v3 패리티 백로그 P1 #16 커맨드 팔레트(Ctrl+K): CommandPalette 컴포넌트(검색+↑↓+Enter+Esc) — 새 세션/실행·중단/녹화 시작·중단/요소선택/설정/테마·언어 전환/세션 이동을 컨텍스트별 노출. useShortcuts 에 Ctrl+K(입력 포커스 무관) 추가, uiStore paletteOpen/settingsOpen lift(사이드바 기어와 공용), 기존 스토어/액션 재사용. 순수 프론트(백엔드 무관, core 0줄). tsc/build 그린.) (§50 신규 — v3 패리티 백로그 P1 잇기: #10 코드 복사 버튼(CodeViewer 헤더 Copy, navigator.clipboard, 순수 프론트) + #11 Library/Initial 블록 카드(GET /sessions/{id}/blocks → get_library_block_code/get_initial_block_code 위임, ChatPanel 상단 read-only 카드 + uiStore selectedBlock 상호배타 선택 + CodePane read-only 표시). core 0줄, test_220 +1.) (§49 신규 — element picker 풀 복원(투명 오버레이 + 실시간 붉은 박스 하이라이트, v2 동등). fix1~9 실측 루프: ⓐ hover rect dict 정규화 ⓑ F3 일시정지(메뉴 펼친 후 선택) ⓒ **마우스 정지 진짜 원인 = CallNextHookEx argtypes 미지정 → x64 lParam(포인터) OverflowError → fix9 로 해결** (fix6~8 GIL 진단은 오진) ⓓ 녹화 시작 시 메인 minimize / 코드 실행 완료 시 메인 focus. **미해결(보류)**: 작업표시줄 위 hover 박스 z-order — Shell_TrayWnd 가 SetWindowPos(HWND_TOPMOST)로도 안 덮임. core 0줄 불변, test_219 가드.) (§48 신규 — element picker 클릭 시 캡처 절충안: pick_pump.py 전역 LL 마우스 후크, POST /pick/click·/pick/cancel.) (§47 신규 — v3↔v2 기능 패리티 백로그 (서브에이전트 3개 교차조사, 격차 22항목을 (A)브리지 미노출/(B)백엔드 없음·순수UI 로 분류 + 우선순위표). (A)유형 P0 일괄 구현 착수: step 삭제/이동/삽입/재생성 + 세션 삭제/이름변경 + from 실행 버튼 + 경고상세/패키지 표시. core 0줄 불변.) (§46 신규 — Phase E 배포 셋업: electron-builder 설정 (win nsis + extraResources pybridge) + main 의 bridgeCommand() packaged/dev 분기 (app.isPackaged 시 frozen ohdo-bridge.exe spawn) + PyInstaller spec/entry (api_server+core onedir freeze, hidden-import 다수). freeze/설치 실측은 사용자 머신 (이 환경 GUI 불가). core/PySide6 0줄 수정.) (§45 신규 — Phase D i18n + 애니메이션: desktop_v3 react-i18next (신규 ko/en 카탈로그, 시스템 locale 감지 + localStorage + 사이드바 언어 토글). UI 한국어 → t()/i18n.t 전환 (UI 한국어 0, 주석만). 핵심 트랜지션 (PickOverlay fade / Toaster·LogConsole slide). core/PySide6 0줄 수정, TS 전용. tsc+build 그린.) (§44 신규 — AI 생성 진행상황 스트리밍: WS /ws/generate 가 generate_step 의 on_progress 콜백("프롬프트 구성 중"/"AI 호출 중 N자")을 실시간 전송. core generate() 토큰 스트리밍 미지원 + agy CLI(PTY) 토큰 스트리밍 불가 → 사용자 결정으로 토큰 대신 진행상황 (core 0줄 수정). ChatPanel POST→WS 전환, 로딩 영역 라이브 진행 표시. test_211 +1.) (§43 신규 — api_server 리팩토링: server.py(568줄)를 deps.py + routes/{health,sessions,steps,pick,recording,execution}.py 로 분리, server.py 는 55줄(state 셋업 + include_router)로 축소. 엔드포인트/동작 100% 동일 (13 routes parity + E2E 검증). 스키마는 server.py 에 하위호환 re-export. 기능 변화 0.) (§42 신규 — **녹화 실제 캡처 fix**: §41 의 "InputHookManager 자체 펌프 스레드" 가정이 틀렸음 — LL 훅은 SetWindowsHookEx 호출 스레드에서 메시지 펌프가 돌아야 콜백 발화. FastAPI 브리지엔 펌프 없음 → 입력 0 캡처 (사용자 실측). fix: `api_server/recording_pump.py` RecordingController 가 전용 스레드에서 start_recording(훅 설치)+PeekMessage 펌프+stop_recording(훅 해제) 수행. E2E 검증: 키입력 6회 → event_count 6 + stop_commit → step 2개 생성 확인. core/PySide6 0줄 수정.) (§41 신규 — TS UI v3 #3 잔여 = 작업 녹화 lifecycle: api_server REST (recording start/status/marker/stop_commit/cancel) + desktop_v3 녹화 버튼/구분점/이벤트 카운트 polling. recorder 의 InputHookManager 자체 pump 스레드로 브리지에서 동작. is_recording/recording_event_count 는 @property (() 호출 금지). core/PySide6 0줄 수정, test_210 +1. Monaco "Loading" CDN/CSP fix 는 §41 에 함께 기록.) (§40 신규 — TS UI v3 Phase B 확장: #1 step 실행 + live 로그 (WS /ws/execute) + #2 코드 편집·저장 (Monaco editable + PUT update_step) + #3 element picker (카운트다운 캡처 /pick) + #4 polish (토스트 + 단축키 + 다크/라이트 테마 토글). core/PySide6 0줄 수정, test_208/209 +2. 다음: 녹화 lifecycle + 사용자 GUI 실측.) (§39 신규 — TS UI v3 Phase B 1차 증분: api_server 에 GET /sessions/{id} + POST /sessions + POST /sessions/{id}/generate (AI 코드 생성, async generate_step 위임) 추가 + settings/prompts 주입. desktop_v3 에 shadcn/ui + Monaco + Discord 3-column (사이드바/채팅/코드뷰어) + AI 생성 루프 (동기 요청+로딩). core/PySide6 0줄 수정, test_207 +1. 다음: 사용자 GUI 실측 (세션 생성 -> 자연어 요청 -> 코드 화면 표시).) (§38 신규 — TS UI v3 Phase A 셋업 완료: `api_server/` FastAPI 브리지 (GET /health + /sessions, 토큰 인증, READY 마커 포트 협상) + `desktop_v3/` Electron 38 + React 19 + TS + Vite 6 + Tailwind + Zustand + TanStack Query 보일러플레이트. core/PySide6 0줄 수정, test_206 회귀 가드 +1. 다음 세션 Phase B 핵심 화면 MVP — 사용자 GUI 실측 `npm run dev` 먼저.) (§37 신규 — TS UI v3 트랙 전환 결정: Electron + React + TS + Tailwind + Zustand, 같은 repo `desktop_v3/`, Discord-like UX. 다음 세션 Phase A 셋업부터 시작. §36 hotfix series 완료 — agy ConPTY 우회 (pywinpty + cmd bat 래퍼) 로 stdout 캡처 성공, CLI AI 일반화 + Gemini→Agy rename + preset UI + test_204/205) (§34 PR-19m +1 = test_202 raw events 사후 재변환 helper + CLI; §33 PR-19l +1 = test_201 generated_code destructive 패턴; §32 PR-19k +1 = test_200 한글 IME pyperclip placeholder; §31 PR-19i +1 = test_199 raw events JSONL 저장; §30 PR-19h +1 = test_198 destructive ⚠️ badge + commit confirm; §29 PR-19c +1 = test_197 idle gap → wait_after_ms 충전; §28 PR-19j +1 = test_195 regenerate in-place fix, PR-19b +1 = test_196 빠른 double-click 감지) (PySide6 단독 `.venv` 기준 — PR-11~18 = +37 + GUI 실측 1차 fix +5 (test_182~186) + PR-19a-g +8 (test_187~194) = 144→194). **2026-05-23~24 PR-19a → PR-19g 7개 fix 모두 완료, 사용자 GUI 실측 검증 통과** — 녹화 + 입력 + 실행 흐름이 처음으로 사용자 의도대로 동작. (a) PR-19a `core/pywinauto_codegen.py` helper 추출 + recorder 통합. (b) PR-19d `Step.element_meta` 보존 + AI 재생성 path adapter (PR-19d 의 hybrid mode 는 미테스트 — 옵션 3 후속). (c) PR-19e `_safe_str_literal` (json.dumps escape) — Win11 메모장 Document name 의 `\r` SyntaxError 회귀 차단 + `_build_connect_block` 이 `process_id` 우선 connect chain (탭 이름만 잡힌 case 처리). (d) **PR-19f modifier 키 인식 — Ctrl+A 등 hotkey 변환** (recorder 가 `GetAsyncKeyState` 로 modifier 캡처 → RawEvent.modifiers 채움; transform 이 `pyautogui.hotkey('ctrl', 'a')` emit). Session.recording_meta list 필드 + commit_recording metadata 보존. (e) **PR-19g UWP `Light Dismiss` / `PopupRoot` noise filter** — 메모장 닫힘 회귀 차단 (실측 v2-새세션-005917). 자세한 §27 신규. **다음 세션 출발점 — P1 옵션 3 실증 결과 분석 (진행 중)** → P2 PR-19h destructive UX / P3 PR-19b F-6 dedup / P4 PR-19c idle wait / P5 PR-19i raw events JSONL / P6 CJK IME. 자세한 §27 끝 "다음 세션 출발점". **2026-05-23 PR-19a 완료 — recorder_transform 코드 품질 1차**: 자세한 §26. **2026-05-20~23 사용자 GUI 실측 1차 — 녹화 lifecycle 6 fix 완료** (test_182~186). 자세한 §24 "다음 세션 출발점" + §25. **2026-05-19 사용자 결정 — TS UI 트랙 진행 순서**: ① GUI 실측 (진행 중) → ② AppService API 보강 → ③ 2~3주 뒤 PR-19 (FastAPI 라우터) + PR-20 (Vite + React + TS, web_ui/). **풀 TS 재작성 X — recorder/element_picker/win_inspector 는 Python 유지**, TS 는 UI 레이어만. **5/19 (오후): ADR 0004 Phase R2 PR-18 완료 — DPI/멀티모니터 안정화. `core/input_hooks.py` 에 `ensure_dpi_awareness()` (SetProcessDpiAwarenessContext PER_MONITOR_AWARE_V2 우선, SHCore SetProcessDpiAwareness fallback) + `get_dpi_for_point(x, y)` (MonitorFromPoint + GetDpiForMonitor) helper 추가. `get_hook_manager()` 가 idempotent 로 매 호출 ensure_dpi_awareness 트리거. drain thread 가 click event 의 `monitor_dpi` 캡처 (RawEvent 새 필드). `recorder_transform` 의 fallback `pyautogui.click(x, y)` 에 비표준 DPI 시 코멘트 첨부 (`# captured at DPI=144 (150%)`). **R2 완료 — PR-16w + PR-17 + PR-18 모두 완료.** 자세한 §24.** **5/19 (오전): PR-17 마이그레이션 모드 event queue + async EFP. LL hook callback 은 RawEvent 생성 + 큐 enqueue 만 (sub-ms, fast return). 별도 drain thread 가 큐를 빼며 element_capture_fn 호출 + session.events 적재. Windows ~300ms LL hook 타임아웃 안전 + 빠른 자동화 스크립트 (Power Automate / pywinauto / AutoHotkey) 입력 따라잡기 가능.** **5/18: ADR 0004 Phase R2 PR-16w 완료 — 창 포커스 자동 경계 (SetWinEventHook EVENT_SYSTEM_FOREGROUND + SKIPOWNPROCESS) + F8 키보드 hook 에서 marker 자동 변환. PR-13 의 `auto_window_focus_boundary` / `enable_f8_marker` TransformOptions 가 비로소 end-to-end 작동 (이전엔 PR-13 에서 분리 로직만 구현되어 있었고 캡처 path 가 없어서 dead code 였음). 자세한 §24.** **5/16: ADR 0004 Phase R1 (5 PR) 완료 후 R2 진입 — PR-16a (element 메타 캡처 갭 메움) 추가. PR-11~15 + PR-16a (`core/element_inspect.py` 신규 + `_do_start_recording` 에서 capture_element_at 을 element_capture_fn 으로 주입 — UIA EFP 로 control_type/name/automation_id/window_title/hwnd/exe_name/rect/is_password_field 채움. 이전엔 element_meta=None 으로 떨어져 recorder_transform 이 좌표 fallback 만 생성하던 갭 해소). 자세한 §24.** **5/13~5/14: ADR 0003 Phase 1+2 완료 — 시크릿 처리 + element placeholder end-to-end (PR-1~10, test_117~144, 28 신규 테스트). 자세한 §23.** **wireframe D1~D26 100% 구현 완료**. 5/7~5/8: Phase 0 인프라 표준화 5/7 sub-phase 완료 — pyproject.toml + uv + pre-commit + ruff (lint+format) + LICENSE (AGPL-3.0) + SPDX 헤더 113 파일 + GitHub Actions CI + .devcontainer. **5/8~5/9: Phase 1 5/5 sub-task 모두 완료** — 저장소 추상화 + UI-Core 분리 (Chunk A 5/8 + Chunk B 5/9) + Pydantic 모델 + 설정 레이어 + Agent 브리지. **5/9 시장 타깃 결정**: 한국 niche → **글로벌 + 한국 dual-locale**. 영어 README + UI/메시지 i18n 작업이 Phase 2 진입 직전 필수. **Phase 2 진입은 docs/commercial_review.md (현 docs_private/ 비공개 보관) GO/NO-GO 게이트 통과 후 결정** (5/9 글로벌 dual-locale 반영 갱신). **5/9~5/10: Phase 1.8 OpenAI 호환 (DeepSeek) 등록 + 코드 생성 품질 루프 — Step A/B + B1+B2+B4 + P4 + P1a/P1b/P3 + G1/G2/G2.5 + G5 (11 unit, test_86~96)**. **5/10~5/11: Phase 1.8 G7 코드 정적 분석 + 사용자 경고 + 재생성 흐름 — G7-A/B/C/D (4 unit, test_97~100)**. **5/11: Phase 1.8 후속 fix 모음 — G4 + G7-E (E1/E2) + G6 + F2 + G7-UX + F1 (7 unit, test_101~106). handoff §16 잔존 갭 #1~#6 + 후속 fix 옵션 6개 모두 완료**. 자세한 §18. **5/12 (오전): Phase 1.9 C-1 i18n 인프라 시작 — core/i18n.py + locale/{en,ko}.json (1 unit, test_107). 또한 5/12 결정: 최종 PySide6 만 사용 (PyQt6 보관). PySide6 port 회귀 가드 11 catch-up (test_97~107). commit b11b980. 자세한 §19.** **5/12 (오후) Plan 1 완료 — PySide6 (LGPL) 메인 전환 (commits 16d5349 → 833174a → f759ebb → d6642f0 + 50b3115). pyside6_port/ → root, PyQt6 → legacy_pyqt6/, PyQt6 dep → optional extra. 자세한 §20.** **5/12 (오후~저녁) Phase 1.9 C-2 완료 — `.gitattributes` 추가 (autocrlf 항구 해결) + ui_v2 i18n 183 catalog 키 (en/ko) + startup locale 자동 감지 + test_108/109 회귀 가드 추가. 8 commits (b8ce57f → 2d9cece). 자세한 §21.** **5/12 (저녁~밤) GUI 핵심기능 테스트 세션 — 사용자가 ohdo (`--ui v2`) 직접 띄워 cmd 실행 / 메모장 / element picker / step 관리 시나리오 반복 테스트하며 발견한 7 fix (test_110~116, **미커밋**): (1) kernel IPC RESULT marker isolation (실패가 ✅ 로 오보고) (2) Windows console-launch 규칙 (cmd/powershell 은 `CREATE_NEW_CONSOLE` 필수 — kernel_worker 가 콘솔 없는 piped subprocess) (3) ui_v2 `self.settings` AttributeError → `self._load_settings()` (4) 재생성 = in-place 대체 (`replaces_step_id` — 새 step 추가 X) (5) F3 picker 후 main window 잔존 → `showMinimized()` (6) step card 🗑 삭제 버튼 복원 (v2 누락) + ⬆⬇ 레이아웃 (7) `delete_step` generated_code chain 재구성 (삭제된 step 코드 잔존 회귀). core 116/116 + scenarios 73/73 그린. 자세한 §22.**
...
| 5/5 (밤) | **AI 환각 import 자동 교정** (사용자 보고: 메모장 세션 OK 후 그룹웨어 브라우저 세션 선택 → 실행 시 `ImportError: cannot import name 'FindBestMatchException' from 'pywinauto.findbestmatch'` → 모든 step cascade fail). 원인: AI 가 pywinauto 의 존재하지 않는 exception 클래스명 (`FindBestMatchException`) 을 환각해서 `from pywinauto.findbestmatch import FindBestMatchException` 생성. 실제 이름은 `MatchError`. 라이브러리 블럭 ImportError → `from selenium...import Options` 등 후속 import 모두 skip → step 1 의 `Options()` NameError, step 2 의 `Application().connect()` 도 except 절에서 `FindBestMatchException` 참조하다 NameError. **윈도우↔브라우저 간섭 X — 단일 세션 내 환각 import 문제**. 3 layer fix: ① **workflow_engine.fix_hallucinated_imports** 신규 — `_HALLUCINATED_PYWINAUTO_NAMES = {'FindBestMatchException': 'MatchError'}` 매핑, 단어 경계 정규식으로 import + except 양쪽 자동 치환. self-멱등 (이미 교정된 코드 재변환 X). 라이브러리 블럭과 delta_code 양쪽에 적용 → 기존 그룹웨어 세션도 즉시 효과. ② **prompts.json guideline 19** — pywinauto 실제 exception 카탈로그 명시 (`MatchError`, `ElementNotFoundError`, `ElementAmbiguousError`, `WindowAmbiguousError`, `WindowNotFoundError`, `AppNotConnected`, `AppStartError`) + 환각 금지 강조 + 불확실하면 `Exception` 사용 권장. ③ test_68 (transform 동작 + 멱등 + 무관 코드 보존), test_69 (prompt 카탈로그 가이드). scenarios 69/69 + core 75/75 그린. |
...
- ⚠️ 그룹웨어 브라우저 세션 (eb17030a) — `FindBestMatchException` cascade 회귀. 다시 실행하면 workflow_engine 의 hallucinated_imports 교정으로 자동 복구돼야 함. **사용자 미검증**.
...
- 기존 그룹웨어 세션은 step_code 가 비어있어 (이전 generate_step 시 백업 패턴 미적용 시점) 일부 fallback 경로만 사용. 새로 생성하는 세션부터 정상 분리.
- ui_v2 GUI 사용자 테스트 미검증 (§6 #4 항목들). 메모장/브라우저 시나리오 외에 다른 워크플로우 (계산기/IDE 등) 검증 필요.

**5/6 새 세션에서 권장 첫 행동**:
1. baseline 테스트 (`core` + `scenarios`) 실행해서 그린 확인
2. 사용자에게 "어제 fix 들 (특히 8 layer 마지막 — 그룹웨어 세션 재실행 + tmp/conversations 로그 확인) 어떻게 됐는지" 물어보기
...
**Phase 1 진행률: 5/5 (100% 완료)** — Phase 2 진입 직전 docs/commercial_review.md (현 docs_private/ 비공개 보관) 재독 필수. 자세한 Chunk B 변경 내역은 §14 참조.
...
docs/commercial_review.md (현 docs_private/ 비공개 보관) — 5/8 작성. ohdo 의 상업적 경쟁력 정직 진단 + Computer Use / UiPath / 기타 RPA 와의 비교 + GO/NO-GO 게이트 제안. **Phase 1 완료 직후 / Phase 2 진입 결정 전 반드시 재독.** Phase 1 까지는 어느 시나리오든 가치 있어 진행 OK.
...
| docs/commercial_review.md (현 docs_private/ 비공개 보관) | 헤더 5/9 갱신 표시, §3 차별성 표 (한국어 UI → i18n dual-locale, 🟢→🟡), §5 SAM anchor 글로벌 + ARR 추정 상향 (비관 0-15K / 중립 20-80K / 낙관 150-500K), §7 GO/NO-GO 게이트 ("한국어 콘텐츠 5+" → "영어 + 한국어 mix"), §9 변경 로그 |
...
## 77. 2026-06-08 멀티모니터 hover 박스 = 디스플레이별 오버레이 (단일 spanning 투명창 한계)

**컨텍스트**: §76(브리지 DPI awareness) 후속 실측 — 보조모니터 요소가 **선택은 됨**(클릭 시 메인
창에 선택 요소 이름 표시) 그러나 마우스 hover 시 **붉은 네모 박스/오버레이가 주모니터에서만** 보이고
보조모니터에선 안 나옴. → 좌표(선택)는 §76 으로 해결됐고, 남은 건 **오버레이 렌더링**.

### 원인 = 단일 투명창이 가상 데스크톱 전체를 덮는 구조 (Electron/Windows 한계)
- picker 오버레이는 `transparent:true` BrowserWindow **1개**가 union(전 디스플레이) bounds 를 덮음.
  Windows 에서 **투명창은 자신이 생성된(주) 디스플레이에만 합성**되고 보조모니터로 확장된 영역은
  렌더 안 됨(특히 다른 DPI). 그래서 박스 CSS 좌표는 맞게 계산돼도(overlay.ts 가 div 절대배치) 보조
  모니터엔 안 그려짐. (overlay 변환수학·렌더러는 정상 — 창 구조가 문제.)

### fix = 디스플레이마다 오버레이 1개 (각 단일 DPI → 정상 렌더)
- **main/index.ts**: `overlayWindow`(1개) → `overlayWindows: {win, displayId}[]`. `createPickOverlay`
  가 `screen.getAllDisplays()` 마다 그 디스플레이 bounds 로 창 1개 생성(전부 투명·클릭통과). `pick:hover`
  가 `BrowserWindow.fromWebContents(event.sender)` 로 폴링한 오버레이를 식별 → `physicalRectToOverlayCss`
  가 요소 중심 DIP 가 **그 창의 디스플레이**일 때만 box 반환(아니면 null) → 각 오버레이는 자기 모니터
  요소만 그림. `pick:start` 가 **모든** 오버레이 HWND 를 `overlayHwnds()` 로 모아 리스트 등록.
- **api_server(core 0줄)**: `pick_pump._overlay_hwnd`(단일) → `_overlay_hwnds`(리스트), 펌프 루프가
  전부 SetWindowPos(HWND_TOPMOST), `set/get_overlay_hwnds` + 하위호환 단일 helper 유지. `/pick/overlay`
  가 `{hwnds:[...]}`(+하위호환 `{hwnd}`) 수용. `_hide_pick_overlay`(§70) 가 등록분 **전부** ShowWindow
  (SW_HIDE) → 보조모니터 요소 캡처에도 붉은 테두리 안 찍힘.
- desktop_v3 + api_server 만, core/ui/ui_v2/main.py **0줄**. test_239 +1 (core **234/234**), tsc/build/ruff 그린.
- **남은 동종**: §60 영역캡처 오버레이(`captureWindow`)도 단일 spanning 투명창 — 보조모니터 드래그
  선택이 동일 증상일 수 있음(이번 미수정, 사용자 영역캡처 실측 시 같은 패턴으로 분리 예정).
- **사용자: 렌더러 재빌드 + 브리지 재시작 후 보조(다른 DPI)모니터에서 hover 붉은 박스가 요소에 정확히
  겹치는지 확인.**

## 76. 2026-06-08 멀티모니터(다른 DPI) 요소 선택 회귀 fix — 브리지 DPI awareness

**컨텍스트**: 사용자 — 출력 모니터 2대 이상 + **각 해상도(DPI)가 다를 때** 요소 선택이 어긋남.
v1 에서 해결했던 문제가 v3 에서 재발. "v3 의 멀티모니터 좌표 코드가 잘 적용됐는지 분석 후 수정."

### 분석 — overlay 변환 수학은 정상, 누락은 **브리지 프로세스의 DPI awareness**
- v3 overlay 좌표 변환(`physicalRectToOverlayCss`, main/index.ts)은 `screen.screenToDipPoint` 로 두
  모서리를 각각 변환 → **모니터별 scaleFactor 가 달라도 DIP 좌표는 정확**(v1 의 cursor-ratio 방식보다
  오히려 견고). 즉 overlay 측 수학은 문제 아님.
- 진짜 원인: **Python 브리지 프로세스가 PER_MONITOR_AWARE_V2 로 설정되지 않음**. v1 은 `main.py:22`
  에서 `SetProcessDpiAwarenessContext(PER_MONITOR_AWARE_V2)` 를 명시 호출했으나, v3 는 브리지가
  **별도 프로세스**(Electron 이 spawn)이고 어디서도 DPI awareness 를 보장하지 않음
  (`pick_pump` 는 자체 LL 후크 설치 — `get_hook_manager()` 의 `ensure_dpi_awareness` 자동 트리거를
  안 탐). DPI-unaware 프로세스는 멀티-DPI 에서 `GetCursorPos`/LL-hook `pt`/UIA `ElementFromPoint`
  좌표가 OS 에 의해 **가상화** → 보조(다른 DPI) 모니터에서 엉뚱한 요소가 잡히고 하이라이트 박스가 어긋남.

### fix — 진입점에서 PER_MONITOR_AWARE_V2 설정 (core 0줄)
- `api_server/__main__.py`: `main()` 의 **첫 동작**으로 `_ensure_bridge_dpi_awareness()` 호출 →
  `core.input_hooks.ensure_dpi_awareness()`(공개 함수) 위임. parser/uvicorn import/create_app/pick·
  capture 보다 먼저, 1회. best-effort(try/except), non-Windows 는 `"unsupported"` 반환(무해), idempotent.
- 효과: 브리지의 모든 좌표(pick 선택·hover 박스·element 캡처 rect)가 **물리 픽셀**로 일관 → Electron
  의 `screenToDipPoint`(물리→DIP) 기대와 정합 → **모든 모니터에서 정확**. §66~70 요소 스크린샷도
  멀티-DPI 보조모니터에서 함께 정상화(보너스). 단일/100% 모니터는 무변(물리==가상).
- 생성코드 실행(pyautogui/pywinauto 클릭)의 DPI 는 별개(win_inspector 템플릿이 자체 처리) — 이번 회귀
  (요소 *선택*)와 무관.
- core/ui/ui_v2/main.py **0줄**(api_server.__main__ + 테스트만). test_238 +1 (core **233/233**), ruff 그린.
- **사용자: 브리지 재시작(또는 재빌드) 후 보조(다른 DPI) 모니터에서 요소 선택 재확인.**

## 75. 2026-05-31 메인 창 stuck-topmost fix (실행 후 alwaysOnTop 해제 안 됨)

**컨텍스트**: 사용자 — ① 전체 실행 후/창 숨김→복원 후 ohdo 메인 창이 **최상위에 박혀** 다른 창을
선택해도 앞으로 안 옴(ohdo 한번 클릭해야 해제). ② step1 만 있는 세션에서 step1 삭제 후 대화창 입력/
선택 안 됨(다른 창 클릭→재클릭해야 됨). 사용자는 둘이 비슷한 원인(창 포커스)으로 추정.

### ① fix — window:focus 의 alwaysOnTop 동기 해제 버그 (main/index.ts)
- 실행 완료 시 `useExecution` 가 `window:focus` IPC 호출(§49 fix5) → 메인 창을 앞으로. 핸들러가
  `setAlwaysOnTop(true) → focus() → setAlwaysOnTop(false)` 를 **한 tick 에 동기** 실행했는데, 일부
  Windows 에서 rapid 토글 시 **WS_EX_TOPMOST 가 안 풀려** 창이 계속 최상위에 박힘(다른 창 못 옴).
- **fix**: `setAlwaysOnTop(true) → focus()` 후 **`setTimeout(…, 250)` 로 topmost 해제**(foreground
  전환이 끝난 뒤 확실히 NOTOPMOST). 창 닫힘 가드. 메인 무수정(프론트 main 프로세스만), tsc/build 그린.

### ② step 삭제 후 입력 불가 — 추정 + 재확인 요청
- 별도 트리거(`window:focus` 안 탐). `onDeleteStep`(ChatPanel:413)이 네이티브 `window.confirm` 사용 →
  Windows 에서 네이티브 다이얼로그 닫힌 뒤 창 입력 포커스가 비는 케이스 의심. **단** 사용자가 ①과
  연관 추정 → ① 수정으로 stuck-topmost 가 사라지면 ②도 해소될 수 있음(실행 후 삭제 시나리오면). →
  **재빌드 후 ② 재현 여부 확인 필요.** 지속 시 window.confirm 을 커스텀 모달로 교체 / 삭제 후 재포커스.

## 74. 2026-05-31 셀렉터 교정 변수명 일반화 (element_click 등도 교정, §72 후속)

**컨텍스트**: 사용자 — 04:48 세션은 탭닫기 동작, 06:52 세션은 마지막 탭닫기 미동작. 두 세션 비교.

### 원인 = §72 교정 regex 가 변수명 `element` 만 매칭
- 두 세션 모두 "x" 픽(element_context 에 `auto_id=CloseButton`, `이름=탭 닫기` 정상 주입).
- **04:48 step4**: AI 가 `element = win.child_window(...)` → §72 regex(`element\s*=...`) 매칭 →
  CloseButton 으로 교정 → **동작**.
- **06:52 step4**: AI 가 **`element_click = win.child_window(control_type="Document"...)`** (변수명 다름)
  → §72 regex 불일치 → **교정 안 됨** → Document(텍스트영역) 클릭 → 탭 안 닫힘.

### fix (selector_enforce.py, core 0줄)
- `_ELEMENT_CALL_RE` 의 LHS 를 `element` 고정 → **`\w+`** 로 일반화: `(\w+\s*=\s*win\.(?:child_window|
  descendants)\()([^()]*?)(\))`. AI 가 `element`/`element_click`/`el` 등 어떤 변수명을 써도 매칭.
  `win.child_window` 는 항상 picker 가 고른 element 해석이라 안전(변수명은 group1 으로 보존, kwargs 만 치환).
- 실측: 06:52 step4 generated_code 에 적용 → changed=True, CloseButton 타겟, compile OK.

### 검증
- **test_232 확장**(core **228/228**): `element_click` 변수명도 교정+변수명 보존 케이스 추가.
  scenarios 73/73 + recording_fixtures 2/2 + ruff 그린. core/PySide6/ui_v2 0줄, 프론트 무변경.
- **사용자: 브리지 재시작 후 06:52 step4(탭닫기) 재생성** → 변수명 무관하게 CloseButton 교정 → 탭 닫힘.

## 73. 2026-05-31 코드 뷰어 step 별 표시 (누적 generated_code → step_code 델타, v2 동등)

**컨텍스트**: 사용자 — v2 는 step 별로 그 step 의 코드만(블록 단위 try/except) 보였는데, v3 는
step 선택 시 **누적 generated_code(이전 step 전부 + import 구문)** 가 다 보임. v2 의 "메모장테스트2"
세션이 참고(각 step_code = 그 step 블록). v3 도 step 별 코드만 보이게.

### 원인 + fix
- **원인**: `desktop_v3/.../App.tsx:45` 가 `code = step.generated_code`(누적: import+이전step전부+마커)를
  CodeViewer 에 넘김. v2 는 step_code(델타=그 step 블록)를 표시. v3 step_code 도 깨끗한 블록 확인.
- **표시(App.tsx)**: `code = step.step_code || step.generated_code`(델타 우선, 비면 폴백). 블록(Library/
  Initial)은 기존대로 block code.
- **편집(routes/steps.py PUT update_step)**: 뷰어가 step_code 를 보여주므로 편집도 step_code 단위.
  이전엔 `generated_code=편집블록 + step_code=편집블록` 으로 덮어 **누적 코드가 그 블록으로 줄고 후속
  step delta 가 깨졌음**. 이제 `step_code=편집 + manually_edited=True` 저장 후 **`_rebuild_generated_code_chain`**
  (library + 각 step step_code 누적, core 의 reorder/delete 동일 패턴)으로 generated_code 재구성 →
  편집이 후속 step 누적코드에도 전파(마커 없이도 run-time diff 가 델타 복원). core 0줄(공개 API 재사용).

### 검증
- **test_233** +1 (core **228/228**): 2-step 세션 step1 편집 PUT → step_code 저장 + step1/step2 누적코드에
  편집 전파 + 옛 코드 잔존 안 함(체인 재구성). test_209(단일 step PUT)도 통과(편집이 양 필드에 반영).
  scenarios 73/73 + recording_fixtures 2/2 + tsc/build/ruff 그린. core/PySide6/ui_v2 0줄.
- **사용자: 브리지 재시작 + 렌더러 재빌드** 후 step 선택 시 그 step 코드만(이전 step·import 도배 없이)
  표시되는지, 편집·저장 후 실행이 정상인지 확인.
