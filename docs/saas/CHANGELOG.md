...
- [x] **배포 후 브라우저 e2e PASS (2026-04-27)** — Vercel 도메인에서 매직링크 sign-in (테스트 계정) → /dashboard 진입 → 신규 실행 폼 → `exec_94da7992767a45faae60c620e58ff669` 자동 생성 → 10.6s 만에 completed → 메타 카드 (1/1/0) + 로그 5라인 (engine/stdout 색 구분) + 폴링 자동 정지 ("자동 새로고침 정지") + 캡처 (0) 섹션 모두 정상 표시. M3.1.3·M3.1.4·M3.1.5 의 dev 격리 이슈 자연 해소 확인.
