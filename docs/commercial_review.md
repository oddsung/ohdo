...
| **Local-first / offline** | AI 호출 외 데이터 외부 송출 0. 컴플라이언스 민감 환경 친화 | 🟢 한국 공공/금융/의료 시장 의미 있음 |
| **한국어 UI + 한국어 RPA 가이드** | 글로벌 도구 대부분 영어 우선 | 🟢 한국 niche 진입 장벽 |
...
### 정직한 SAM (Serviceable Available Market)
- **글로벌 dev-focused RPA**: ~50-100M USD/yr (작음, 그러나 Cursor/Cody 같은 dev tool 가능성)
- **한국 dev RPA**: ~5-10M USD/yr (매우 작음, 그러나 경쟁 적음)
- 비개발자 RPA = ohdo 진입 불가 (UiPath 영역)

### 1-2년 내 ARR 현실 추정
- 비관: $0-$10K (개인 사용자 GitHub stars 만)
- 중립: $10K-$50K (한국 niche + 일부 SaaS 유료)
- 낙관: $100K-$300K (오픈소스 traction → 컨설팅/엔터프라이즈)
...
### Phase 2 진입 GO/NO-GO 게이트 (제안)

다음 모두 충족 시 → GO (Phase 2 SaaS 백엔드 진입):
- [ ] GitHub Stars 500+
- [ ] Discord/이메일 "유료여도 쓰겠다" 응답 5명+
- [ ] 한국어 콘텐츠 노출 (블로그 5+ post 또는 유튜브 2+ 영상) → 측정 가능한 traffic
