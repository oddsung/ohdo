...
class V2Signals(QObject):
    log = pyqtSignal(str)
    step_done = pyqtSignal(int, bool, str)  # step_id, success, message
    ...
    request_auto_run = pyqtSignal(int)
...
class Toast(QFrame):
    ...
    closed = pyqtSignal(object)  # ToastManager 가 stack 정리용으로 받음
...
class CardDropContainer(QWidget):
    ...
    step_reorder_drop = pyqtSignal(int, int)  # from_step_id, target_step_id
...
class StepCardV2(QFrame):
    ...
    regenerate_requested = pyqtSignal(int, str)  # step_id, user_request
    # D23: step 순서 변경 (⬆⬇ 버튼 fallback). drag-drop 은 후속 슬라이스.
    reorder_requested = pyqtSignal(int, str)  # step_id, "up" | "down"
    ...
    regenerate_with_warnings_requested = pyqtSignal(int)  # step_id
