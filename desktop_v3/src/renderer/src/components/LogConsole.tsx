...
export function LogConsole() {
  ...
  return (
    <div className="flex h-44 flex-col border-t border-black/30 bg-[#1a1b1e]">
      <header className="flex h-8 items-center justify-between border-b border-black/30 px-3">
        ...
        <div className="flex items-center gap-1 text-zinc-400">
          <Button size="icon" variant="ghost" className="h-6 w-6" title="지우기" onClick={clearLogs}>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2 font-mono text-xs leading-relaxed">
        {logs.length === 0 && <p className="text-zinc-500">실행하면 로그가 여기에 표시됩니다.</p>}
        {logs.map((l) => (
          <div
            key={l.id}
            className={
